...
export const PLANNER_SYSTEM_PROMPT: string = [
  ...
  "- EVERY node needs at least one acceptance criterion that carries an",
  "  executable `check` — a goal criterion id it can actually satisfy. A node",
  "  whose only criteria are prose cannot be shown done, and the run will still",
  "  HALT it for failing one; that is a rejected plan. If no goal criterion fits",
  "  a node, the node is not independently verifiable: merge it into the node",
  "  whose criterion covers its work. NEVER write a `check` of your own, and",
  "  never phrase a criterion around evidence the node cannot produce itself",
  "  (an external review, someone else's sign-off, a claim in a report).",
  "- PRESERVE EVERY BACKTICKED ARTIFACT LITERAL. Any repo artifact the goal wraps",
  "  in backticks — a path, a directory, a filename, a field or symbol name, a",
  "  flag, an exact value — is mandated, and the node that owns it must carry it",
  "  VERBATIM, character for character, in its goal or in one of its acceptance",
  "  criteria. This is checked automatically: an artifact literal that survives",
  "  nowhere in the plan is a rejected plan, however good the prose is. Do not",
  "  paraphrase, pluralize, abbreviate, re-case, or summarize these.",
  "- Restate the goal's CONSTRAINTS in the goal of every node they bind — tooling",
  "  rules, forbidden dependencies, what must not be written. A node run sees its",
  "  own goal, not the chain's; a constraint you leave behind is a constraint the",
  "  executor never receives.",
