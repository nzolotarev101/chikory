...
describe("judge pass telemetry (WP-134)", () => {
  ...
  it("emits chikory.judge.pass with verdict/cost/evidence attrs; maxCostShare breach warns + flags", async () => {
    ...
    const spec: TaskSpec = {
      ...
      // Judge share will exceed 0.5 against the seeded step spend → breach.
    ...
    try {
      ...
      // F-230: the share is only meaningful against real EXECUTOR spend. Seed a
      // priced step so this stays a genuine breach and not the keyless-executor
      // case (covered separately below).
      const ref = { id: "a".repeat(64), kind: "diff" as const, bytes: 0, summary: "seed" };
      journal.append({
        kind: "step",
        payload: {
          stepIndex: 0,
    ...
    // Below 1 because the seeded step spend is real — this is a genuine breach,
    // not the F-230 keyless-executor case where the share is 1.0 by construction.
    expect(attrs["cost.share"]).toBeGreaterThan(0.5);
    expect(attrs["cost.share"]).toBeLessThan(1);
  ...
  it("F-230: a KEYLESS executor ($0 step spend) does not trigger the JD-7 warning", async () => {
    // Same setup, minus the seeded step spend — the dogfood configuration, where
    // `agy`/`codex`/`claude-code` authenticate by CLI OAuth and cost $0 on the
    // wire. The share is then 1.0 by construction and the warning's own advice
    // ("consider a larger cadence") cannot move it; dogfood-121 printed it 12
    // times in one chain. The span still records the ratio — only the console
    // noise goes away.
    exporter.reset();
    const server: Server = createServer((req, res) => {
      req.on("data", () => undefined);
    ...
    const runId = "run-telemetry-keyless";
    const dataDir = await mkdtemp(join(tmpdir(), "chikory-telemetry-keyless-"));
    cleanups.push(() => rm(dataDir, { recursive: true, force: true }));
    const ws = workspaceDir(dataDir, runId);
    await mkdir(ws, { recursive: true });
    await execFileAsync("git", ["init", "-q", ws]);
    ...
    const compat = { provider: "openai-compat" as const, model: "fake-judge" };
    const spec: TaskSpec = {
      name: "telemetry-keyless",
      goal: "exercise judge telemetry with a keyless executor",
      repos: [{ url: "unused", writable: true }],
      acceptanceCriteria: [{ id: "AC-1", description: "anything" }],
      budgetUsd: 100,
      maxSteps: 2,
    ...
    // The telemetry is unchanged — the ratio is still measured and flagged.
    const attrs = exporter.getFinishedSpans().filter((s) => s.name === SPAN_JUDGE_PASS)[0]!
      .attributes;
    expect(attrs["cost.share"]).toBe(1);
    expect(attrs["cost.share.breached"]).toBe(true);
    // …but the operator is not told 12 times to fix something they cannot fix.
    expect(warnSpy).not.toHaveBeenCalledWith(expect.stringContaining("maxCostShare"));
  });
});
