...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)**; **P2 underway**. Sixteen dogfood campaigns have reached first-attempt SUCCESS. Dogfood-015 landed WP-219's `readyNodes` core (`40ada45`) and closed F-21 through WP-226. Dogfood-016 (`docs/reports/dogfood-016.md`, run `run-2418f473`) delivered `hasDependencyCycle`, harvested byte-identically and staged; it surfaced F-22 (serialize Devbox commands), F-23 (terminal-boundary journal transition race -> WP-227), and F-24 (explicit verifier command fixed). Next: dogfood-017 / WP-227. · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing. ~~WP-217~~, ~~WP-218 slice 1~~, ~~WP-220~~, ~~WP-222 slice 1~~, ~~WP-223 initial fix~~, ~~WP-224~~, ~~WP-225~~, ~~WP-209 trace slices~~, ~~WP-208 pure delivery slices~~, ~~WP-219 S1 contracts~~, ~~WP-219 `readyNodes`~~, ~~WP-219 `hasDependencyCycle`~~, and ~~WP-226~~ are delivered. **Next: WP-227** (dogfood-016 F-23, final journal drain; dogfood-017 ready), then WP-221 completion-signal wiring, WP-218 token-budget enforcement, and the remaining WP-219 planner/meta-judge/chain-executor slices. Rationale: `docs/reports/dogfood-002.md` through `docs/reports/dogfood-016.md`.
...
| WP-227 | Final journal drain before terminal return | 🟢 | **Queued (dogfood-016 F-23; dogfood-017 ready).** Refactor `followRun` journal draining into an idempotent closure and drain once more after `status()` returns terminal, before returning. Deterministic regression appends ESCALATE during `status()` and asserts the AWAITING_APPROVAL line renders exactly once. |
