...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)**; **P2 underway**. Sixteen dogfood campaigns reached first-attempt SUCCESS; dogfood-017 is the **first FAILED run — and the strongest thesis data point yet**: WP-227 had already been hand-landed (`26b9964`) so the spec ran redundantly, the executor narrated completion over an empty diff, all 3 acceptance checks + 4 rubric items passed, and the structurally-different judge still caught it and ⚠ ESCALATEd (`docs/reports/dogfood-017.md`, run `run-19c25609`). Surfaced F-25 (launch baseline-satisfied precheck -> WP-228), F-26 (executor empty-diff completion claim -> raises WP-221), F-27 (ESCALATE reason absent from `--watch` -> WP-229). Dogfood-016 surfaced F-22/F-23(->WP-227)/F-24. Next: dogfood-018 / WP-229. · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing. ~~WP-217~~, ~~WP-218 slice 1~~, ~~WP-220~~, ~~WP-222 slice 1~~, ~~WP-223 initial fix~~, ~~WP-224~~, ~~WP-225~~, ~~WP-209 trace slices~~, ~~WP-208 pure delivery slices~~, ~~WP-219 S1 contracts~~, ~~WP-219 `readyNodes`~~, ~~WP-219 `hasDependencyCycle`~~, ~~WP-226~~, and ~~WP-227~~ (hand-landed `26b9964`) are delivered. **Next: WP-229** (dogfood-017 F-27, surface the ESCALATE reason in `--watch`; dogfood-018 ready), then WP-221 completion-signal wiring (raised in priority by dogfood-017 F-26 — the executor narrated a done deliverable over an empty diff), WP-228 launch baseline-satisfied precheck (dogfood-017 F-25), WP-218 token-budget enforcement, and the remaining WP-219 planner/meta-judge/chain-executor slices. Rationale: `docs/reports/dogfood-002.md` through `docs/reports/dogfood-017.md`.
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Queued (dogfood-003 F-11).** Fifteen probe data points now span 5.8%-35.1% of run cost; dogfood-016 added 59k input tokens, 29s, $0.0832, and 7.6%. The `StepRecord.claimsComplete?: boolean` contract landed in `d56f35a`. Remaining: adapters populate it and the WP-217 trigger ORs it in so the productive step is judged directly. |
...
| WP-229 | Surface the ESCALATE reason in `--watch` | 🟢 | **Next up (dogfood-017 F-27; dogfood-018 ready).** `followRun`'s AWAITING_APPROVAL line (`commands.ts:119-127`) drops `verdict.escalateReason`; the live operator sees `⚠ ESCALATE` with no reason and must read `journal.db`/`chikory trace` to decide approve-vs-reject. Render the escalate reason (and top concern) on the watch line. Pure CLI renderer slice, no contracts change. |
