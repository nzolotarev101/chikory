...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)**; **P2 underway**. Seventeen dogfood campaigns reached first-attempt SUCCESS (dogfood-017 the lone FAILED — a true-positive judge ESCALATE on a redundant spec). **dogfood-018 (`run-59115f35`) delivered WP-229 cleanly** — `followRun` now renders `judge escalated: <reason>` on the `--watch` stream before the AWAITING_APPROVAL line; diff byte-for-byte to spec, 3/3 AC + 4/4 rubric PROCEED, harvested IDENTICAL (`docs/reports/dogfood-018.md`). **F-27 closed.** Surfaced F-28 (specs over-prescribed to the keystroke under-test the thesis → DOGFOODING §3 guidance, no code WP); F-11 recurred at 34.8 % of run cost (seventeenth data point, top of range). Next: dogfood-019 / WP-221 pure trigger half (OR `claimsComplete` into the WP-217 empty-diff judge trigger — directly removes the F-11 probe step), then WP-221 adapter half, WP-228, WP-218 token gate. · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing. ~~WP-217~~, ~~WP-218 slice 1~~, ~~WP-220~~, ~~WP-222 slice 1~~, ~~WP-223 initial fix~~, ~~WP-224~~, ~~WP-225~~, ~~WP-209 trace slices~~, ~~WP-208 pure delivery slices~~, ~~WP-219 S1 contracts~~, ~~WP-219 `readyNodes`~~, ~~WP-219 `hasDependencyCycle`~~, ~~WP-226~~, ~~WP-227~~ (hand-landed `26b9964`), and ~~WP-229~~ (dogfood-018 `run-59115f35`, F-27 closed) are delivered. **Next: WP-221 pure trigger half** (dogfood-019 ready — extract the completion-milestone predicate and OR in `claimsComplete` so the *productive* step is judged directly; directly removes the F-11 probe step that cost dogfood-018 34.8 %), then WP-221 adapter half (adapters populate `claimsComplete`), WP-228 launch baseline-satisfied precheck (dogfood-017 F-25), WP-218 token-budget enforcement, and the remaining WP-219 planner/meta-judge/chain-executor slices. Rationale: `docs/reports/dogfood-002.md` through `docs/reports/dogfood-018.md`.
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Next up — pure trigger half (dogfood-019 ready).** Seventeen probe data points now span 5.4%-35.1% of run cost; dogfood-018 added 339k input tokens, $0.4457, and **34.8%** (top of range). The `StepRecord.claimsComplete?: boolean` contract landed in `d56f35a` (already documents "OR'd into the WP-217 empty-diff trigger"). **Slice A (dogfood-019):** extract `agent-loop.ts:211`'s inline `completionMilestone` into a pure exported `isCompletionMilestone(record)` returning `status === "SUCCESS" && (diffRef.bytes === 0 || claimsComplete === true)`; call it from the loop; unit-test the predicate in isolation. Behavior unchanged for today's runs (claimsComplete still absent). **Slice B (later):** adapters populate `claimsComplete` from the executor's terminal output so the *productive* step is judged directly and the F-11 probe step disappears. |
