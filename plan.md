...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)**; **P2 underway**. Nineteen dogfood campaigns reached first-attempt SUCCESS (dogfood-017 the lone FAILED). **dogfood-020 (`run-3575ba23`) delivered WP-230** — `typecheck` now runs a second `tsc -p tsconfig.test.json` pass so `test/**` is type-checked; verified it trips on a bad fixture (`TS2353`). **F-29 closed** — the cheap gate the judge sits on is now sound. Surfaced **F-30** (the same spec was launched twice ~11 min apart, ~$1 wasted; operator ceremony, no WP — WP-228's precheck wouldn't catch an unlanded duplicate). F-11 recurred at 33.2 % (nineteenth data point). **Design walls now cleared by hand (2026-06-14)** to break the polish streak: **(B, done)** the WP-221 completion-marker protocol — `COMPLETION_MARKER` exported from `executors/step.ts` + the `renderStepPrompt` instruction telling the agent to emit it — landed by hand; **(A, decided)** the WP-218 token HALT-event shape is fixed (additive `cause?: "usd"|"tokens"` + optional `remainingTokens?` on `budget_event`, token numbers ride existing `details`). Next: **dogfood-021 = WP-221 Slice B** (runner detects the marker → populates `claimsComplete` → `isCompletionMilestone` already fires it → the F-11 probe step retires — the cost win). The deferred WP-218 pure token-budget math slice is preserved (unscheduled) in the WP-218 row. · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing. ~~WP-217~~, ~~WP-218 slice 1~~, ~~WP-220~~, ~~WP-222 slice 1~~, ~~WP-223 initial fix~~, ~~WP-224~~, ~~WP-225~~, ~~WP-209 trace slices~~, ~~WP-208 pure delivery slices~~, ~~WP-219 S1 contracts~~, ~~WP-219 `readyNodes`~~, ~~WP-219 `hasDependencyCycle`~~, ~~WP-226~~, ~~WP-227~~ (hand-landed `26b9964`), ~~WP-229~~ (dogfood-018 `run-59115f35`, F-27 closed), ~~WP-221 Slice A~~ (pure trigger half, dogfood-019 `run-d836635b`), and ~~WP-230~~ (typecheck covers `test/**`, dogfood-020 `run-3575ba23`, F-29 closed) are delivered. **Next: WP-221 Slice B** (dogfood-021 ready — the cost win). The completion-marker protocol wall is **cleared by hand**: `COMPLETION_MARKER` in `executors/step.ts` + the `renderStepPrompt` instruction. dogfood-021 implements the runner-side consumption — `claimsCompleteFromSummary` in `step.ts` sets `StepRecord.claimsComplete`, which the already-landed `isCompletionMilestone` (Slice A) ORs into the WP-217 trigger, retiring the F-11 probe step that has taxed every run 5.4–35.1 %. Then: WP-218 gate wiring (**HALT-event shape now decided** — additive `cause?`/`remainingTokens?`; a future dogfood), the deferred **WP-218 pure token-budget math** (unscheduled, see WP-218 row — `estimateNextStepTokens`/`tokenBudgetBreached`), WP-228 launch baseline precheck (dogfood-017 F-25), and the WP-219 S2 planner/chain-executor slices (still need the planner contract designed). Rationale: `docs/reports/dogfood-002.md` through `docs/reports/dogfood-020.md`.
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Slice A done** (dogfood-019 `run-d836635b`, harvested + staged on `main` pending commit — `docs/reports/dogfood-019.md`): `agent-loop.ts:211`'s inline `completionMilestone` is now the pure exported `isCompletionMilestone(record)` (`src/workflow/judge-trigger.ts`) returning `status === "SUCCESS" && (diffRef.bytes === 0 || claimsComplete === true)`, unit-tested in isolation (six cases); behavior unchanged on today's runs (nothing sets `claimsComplete` yet). Nineteen probe data points span 5.4%-35.1%; dogfood-020 paid 33.2%. **Slice B (dogfood-021, next).** Completion-marker protocol **cleared by hand** (2026-06-14): `COMPLETION_MARKER = "CHIKORY_TASK_COMPLETE"` exported from `executors/step.ts`, and `renderStepPrompt` (`executors/prompt.ts`) instructs the agent to end its final message with that line iff the task is fully complete (vendor-neutral — detected centrally from `parsed.summary`, no per-adapter change). dogfood-021 implements the consumption: pure `claimsCompleteFromSummary(summary)` in `step.ts` sets `StepRecord.claimsComplete` on the SUCCESS branch; `isCompletionMilestone` (Slice A) already ORs it in → the productive step is judged directly, F-11 probe step retires. End-to-end agent-loop proof (probe step gone) is a documented follow-up. |
