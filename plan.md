...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)**; **P2 underway**. Twenty dogfood campaigns reached first-attempt SUCCESS (dogfood-017 the lone FAILED). **dogfood-021 (`run-91eced6b`) delivered WP-221 Slice B** — `claimsCompleteFromSummary` in `executors/step.ts` reads the executor's completion marker out of `parsed.summary` and sets `StepRecord.claimsComplete` on the SUCCESS branch; Slice A's `isCompletionMilestone` already ORs it into the WP-217 trigger. **The F-11 cost win is now complete in code** (marker protocol + Slice A trigger + Slice B consumption all landed). Delivery byte-`IDENTICAL`, 3 files, no contract change (`claimsComplete?` already on `StepRecord`). **No new friction; F-30 did not recur** (single clean launch). The validating signal is in the journal: step 1 (productive) ended its summary with `CHIKORY_TASK_COMPLETE` on its own line — so once this lands, that summary sets `claimsComplete === true` and the probe step never runs. F-11 was paid one last time (twentieth data point, 26.6 %) because the run's own loop ran pre-Slice-B HEAD code. **F-11 closure is now an observation away — watch the next run for the retired probe step.** Next: **dogfood-022 = WP-218 pure token-budget math** (`estimateNextStepTokens`/`tokenBudgetBreached` in `runner/budget.ts`, mirroring the USD gate; contract `TaskSpec.budgetTokens` already landed) — the original dogfood-021 slice, reprioritized behind the F-11 win on 2026-06-14, now the best remaining pure, contract-landed slice. · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing. ~~WP-217~~, ~~WP-218 slice 1~~, ~~WP-220~~, ~~WP-222 slice 1~~, ~~WP-223 initial fix~~, ~~WP-224~~, ~~WP-225~~, ~~WP-209 trace slices~~, ~~WP-208 pure delivery slices~~, ~~WP-219 S1 contracts~~, ~~WP-219 `readyNodes`~~, ~~WP-219 `hasDependencyCycle`~~, ~~WP-226~~, ~~WP-227~~ (hand-landed `26b9964`), ~~WP-229~~ (dogfood-018 `run-59115f35`, F-27 closed), ~~WP-221 Slice A~~ (pure trigger half, dogfood-019 `run-d836635b`), ~~WP-230~~ (typecheck covers `test/**`, dogfood-020 `run-3575ba23`, F-29 closed), and ~~WP-221 Slice B~~ (runner consumes the marker → `claimsComplete`, dogfood-021 `run-91eced6b` — the F-11 cost win, now complete in code; probe-retirement is one observation away) are delivered. **Next: WP-218 pure token-budget math** (dogfood-022 ready). The slice adds `estimateNextStepTokens`/`tokenBudgetBreached` to `runner/budget.ts`, the token twin of the USD `estimateNextStepCost`/`budgetBreached` — pure, contract `TaskSpec.budgetTokens` already landed, no contracts change. This was the original dogfood-021 slice, reprioritized behind the F-11 win on 2026-06-14 (preserved in the WP-218 row); now the best remaining pure, contract-landed slice. Then: WP-218 gate wiring (**HALT-event shape now decided** — additive `cause?`/`remainingTokens?`; a future dogfood, needs a fake-executor seam designed first), WP-228 launch baseline precheck (dogfood-017 F-25), and the WP-219 S2 planner/chain-executor slices (still need the planner contract designed). The F-11 end-to-end probe-retirement proof (fake executor emits the marker through the Temporal loop, assert no probe step) is a hand-design follow-up, not a pure dogfood. Rationale: `docs/reports/dogfood-002.md` through `docs/reports/dogfood-021.md`.
