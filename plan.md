...
**Queue order**: dogfood findings outrank the original listing — ~~WP-217~~ (done), ~~WP-218 slice 1~~ (done, `2a4dd21`), ~~WP-220~~ (done, commit pending review), ~~WP-222 slice 1~~ (done, `18fae43`), ~~WP-223~~ (done, commit pending review), ~~WP-224~~ (done, commit pending review), ~~WP-225~~ (done, dogfood-009, commit pending review), ~~WP-209 slice 1~~ (done, dogfood-010, commit pending review), ~~WP-209 slice 2~~ (done, dogfood-011, commit pending review — both SE-3 footer halves now render), ~~WP-208 slice 1~~ (done, dogfood-012, harvested uncommitted on `main` — pure `notificationsFor` derivation), then WP-219's ADR; WP-221 + WP-218 slice 2 (`budget_tokens`) ride with the next architect-reviewed contracts PR; the next dogfoodable 🟢/no-contracts slice is dogfood-013 (WP-208 slice 2 pure half — the Slack-payload formatter; WP-209's remaining OTel-metrics work is no longer pure-renderer — see §6). Rationale: `docs/reports/dogfood-002.md` (F-8/F-9/F-10) + `docs/reports/dogfood-003.md` (F-11/F-12/F-13) + `docs/reports/dogfood-004.md` (F-14/F-15/F-16) + `docs/reports/dogfood-005.md` (F-17/F-18) + `docs/reports/dogfood-006.md` (no new F-n — recurrences only) + `docs/reports/dogfood-007.md` (F-19) + `docs/reports/dogfood-008.md` (no new F-n — recurrences only) + `docs/reports/dogfood-009.md` (no new F-n — F-19 closed, recurrences only) + `docs/reports/dogfood-010.md` (no new F-n — recurrences only, F-11 mid-spread 16.1 %) + `docs/reports/dogfood-011.md` (no new F-n — recurrences only, F-11 record-high 34.3 %) + `docs/reports/dogfood-012.md` (no new F-n — recurrences only, F-11 within-spread 25.1 %).
...
| WP-208 | Checkpoint notifications | 🟢 | **Slice 1 done** (dogfood-012 run `run-ea31f96c`, diff verified, harvested uncommitted on `main` — `docs/reports/dogfood-012.md`): pure `notificationsFor(entries, policy)` in `src/runner/notifications.ts` maps a `JournalEntry[]` + `NotificationPolicy` to the ordered `Notification[]` the loop should fire (verdict→escalate/milestone, terminal→terminal; `policy.on` filters; every other kind skipped); two NEW files (`notifications.ts` 47 lines / `notifications.test.ts` 2 tests), no contracts/types/schema/journal-kind change, no call-site. Remaining slices: **(2, next 🟢 dogfood-013)** the pure Slack-payload formatter (`Notification` → `{ text }`); **(3, non-pure, deferred)** side-effectful delivery (Slack webhook via `slackWebhookEnv`, desktop ping) + the runner call-site that invokes `notificationsFor` and dispatches. |
