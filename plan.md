...
**Queue order**: dogfood findings outrank the original listing — ~~WP-217~~ (done), ~~WP-218 slice 1~~ (done, `2a4dd21`), ~~WP-220~~ (done, commit pending review), ~~WP-222 slice 1~~ (done, `18fae43`), ~~WP-223~~ (done, commit pending review), ~~WP-224~~ (done, commit pending review), ~~WP-225~~ (done, dogfood-009, commit pending review), then **WP-209** (process metrics in trace — dogfood-010, the next 🟢/no-contracts item; the queue head WP-219/WP-221/WP-218-s2 are all design- or contract-gated, hand-done first per TASK-PROTOCOL §4), then WP-219's ADR; WP-221 + WP-218 slice 2 (`budget_tokens`) ride with the next architect-reviewed contracts PR. Rationale: `docs/reports/dogfood-002.md` (F-8/F-9/F-10) + `docs/reports/dogfood-003.md` (F-11/F-12/F-13) + `docs/reports/dogfood-004.md` (F-14/F-15/F-16) + `docs/reports/dogfood-005.md` (F-17/F-18) + `docs/reports/dogfood-006.md` (no new F-n — recurrences only) + `docs/reports/dogfood-007.md` (F-19) + `docs/reports/dogfood-008.md` (no new F-n — recurrences only) + `docs/reports/dogfood-009.md` (no new F-n — F-19 closed, recurrences only).
...
| WP-209 | Process metrics | 🟢 | Components-over-time, issues-found:changes-made ratio, feedback frequency — in trace output + OTel metrics. Feedback-frequency already renders (WP-142). **Slice 1 queued as dogfood-010 (spec ready)**: add the issues-found : changes-made ratio (failed criteria/rubric + concerns across judge passes : steps with non-empty diffs) to the `chikory trace` totals footer — pure renderer enhancement over existing journal entries, no contracts change. Components-over-time + OTel metrics deferred to later slices. |
