...
**Queue order**: dogfood findings outrank the original listing — ~~WP-217~~ (done), ~~WP-218 slice 1~~ (done, `2a4dd21`), ~~WP-220~~ (done, commit pending review), ~~WP-222 slice 1~~ (done, `18fae43`), ~~WP-223~~ (done, commit pending review), ~~WP-224~~ (done, commit pending review), ~~WP-225~~ (done, dogfood-009, commit pending review), ~~WP-209 slice 1~~ (done, dogfood-010, commit pending review), then **WP-209 slice 2** (components-over-time in trace — dogfood-011, the next 🟢/no-contracts renderer slice; the queue head WP-219/WP-221/WP-218-s2 are all design- or contract-gated, hand-done first per TASK-PROTOCOL §4), then WP-219's ADR; WP-221 + WP-218 slice 2 (`budget_tokens`) ride with the next architect-reviewed contracts PR. Rationale: `docs/reports/dogfood-002.md` (F-8/F-9/F-10) + `docs/reports/dogfood-003.md` (F-11/F-12/F-13) + `docs/reports/dogfood-004.md` (F-14/F-15/F-16) + `docs/reports/dogfood-005.md` (F-17/F-18) + `docs/reports/dogfood-006.md` (no new F-n — recurrences only) + `docs/reports/dogfood-007.md` (F-19) + `docs/reports/dogfood-008.md` (no new F-n — recurrences only) + `docs/reports/dogfood-009.md` (no new F-n — F-19 closed, recurrences only) + `docs/reports/dogfood-010.md` (no new F-n — recurrences only, F-11 mid-spread 16.1 %).
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Queued (dogfood-003 F-11).** WP-217's empty-diff inference still costs one full probe step (~155–245k input tokens/slice: 155k in run-2899005b, 211k in run-b2f3504d, 158k in run-9edbcd28, 245k = **$0.33, 15.3% of run cost** in run-34926e85 — the first priced probe; 156k = $0.21, 14.8% in run-559ea904; 136k = $0.19, **18.9%** in run-22b337a9; 252k = **25.4%** in run-86c4b628, the record high; 34k = $0.05, **5.8%** in run-841bc838, the record low; 144k = $0.196, **16.1%** in run-c9df353b — spread now **5.8 %–25.4 %** over nine data points, tracking executor discretion on suite re-run) before the trigger can fire. Note: the milestone trigger has coincided with the cadence tick in every campaign since WP-217 landed — it has yet to fire off-cadence in production; WP-221 is what makes it pay off. Add `claimsComplete: boolean` to `StepRecord` (contracts PR, architect-reviewed — ride the next contracts change), populate from executor adapters' final-summary signal, OR it into the WP-217 trigger so the *productive* step is judged directly. |
