...
**Queue order**: dogfood findings outrank the original listing — ~~WP-217~~ (done), ~~WP-218 slice 1~~ (done, `2a4dd21`), ~~WP-220~~ (done, commit pending review), ~~WP-222 slice 1~~ (done, commit pending review), WP-223 (watch fidelity — dogfood-007, spec ready) + WP-224 (`land --verify`, both small), then WP-219's ADR; WP-221 + WP-218 slice 2 (`budget_tokens`) ride with the next architect-reviewed contracts PR. Rationale: `docs/reports/dogfood-002.md` (F-8/F-9/F-10) + `docs/reports/dogfood-003.md` (F-11/F-12/F-13) + `docs/reports/dogfood-004.md` (F-14/F-15/F-16) + `docs/reports/dogfood-005.md` (F-17/F-18) + `docs/reports/dogfood-006.md` (no new F-n — recurrences only).
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Queued (dogfood-003 F-11).** WP-217's empty-diff inference still costs one full probe step (~155–245k input tokens/slice: 155k in run-2899005b, 211k in run-b2f3504d, 158k in run-9edbcd28, 245k = **$0.33, 15.3% of run cost** in run-34926e85 — the first priced probe; 156k = $0.21, 14.8% in run-559ea904, the fifth data point) before the trigger can fire. Note: the milestone trigger has coincided with the cadence tick in both campaigns since WP-217 landed — it has yet to fire off-cadence in production; WP-221 is what makes it pay off. Add `claimsComplete: boolean` to `StepRecord` (contracts PR, architect-reviewed — ride the next contracts change), populate from executor adapters' final-summary signal, OR it into the WP-217 trigger so the *productive* step is judged directly. |
| WP-222 | Executor subprocess env allowlist | 🟡 | **Slice 1 done** (dogfood-006 run `run-559ea904`, diff verified, commit pending review — `docs/reports/dogfood-006.md`): `scrubExecutorEnv` in `src/executors/env.ts`; codex keeps only `OPENAI_API_KEY`, claude-code only `ANTHROPIC_API_KEY`; spawn-level leak tests via `FAKE_ECHO_ENV` fake bin. Self-falsified live: the F-14 leak fired inside the delivering run's own executor steps (shim HTTP 500 ×2); closure confirmed by the next campaign's clean steps. Remaining slice: explicit TaskSpec pass-through opt-in — contracts, rides WP-221's PR. |
| WP-223 | Watch renders journal transitions, never sampled state | 🟡 | **Next up (dogfood-004 F-15; recurred ×2 in dogfood-006 — dogfood-007 spec ready).** `run --watch` missed printing `SUSPENDED at the budget cap` when a resume landed between polls — a budget suspension can vanish from watch output (CG-2 transparency break), and the assertion flake aborted `devbox run harvest` pre-commit. Second flavor seen in dogfood-006: the `AWAITING_APPROVAL` line flaked in the executor's workspace full-suite run, and `budget halt` flaked again on the host during review (passes in isolation). Scope: derive *both* transition lines from journal entries (`budget_event` halt + ESCALATE verdict — already durably journaled; no contracts change) so every state change prints exactly once; de-flakes `cli.test.ts`. |
