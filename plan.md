...
**Queue order**: dogfood findings outrank the original listing — ~~WP-217~~ (done), ~~WP-218 slice 1~~ (done, `2a4dd21`), ~~WP-220~~ (done, commit pending review), ~~WP-222 slice 1~~ (done, `18fae43`), ~~WP-223~~ (done, commit pending review), ~~WP-224~~ (done, commit pending review), WP-225 (F-19 test de-flake — dogfood-009, spec ready), then WP-219's ADR; WP-221 + WP-218 slice 2 (`budget_tokens`) ride with the next architect-reviewed contracts PR. Rationale: `docs/reports/dogfood-002.md` (F-8/F-9/F-10) + `docs/reports/dogfood-003.md` (F-11/F-12/F-13) + `docs/reports/dogfood-004.md` (F-14/F-15/F-16) + `docs/reports/dogfood-005.md` (F-17/F-18) + `docs/reports/dogfood-006.md` (no new F-n — recurrences only) + `docs/reports/dogfood-007.md` (F-19) + `docs/reports/dogfood-008.md` (no new F-n — recurrences only).
...
| WP-225 | De-flake the WP-217 milestone test (durable-state waitFor) | 🟢 | **Next up (dogfood-007 F-19 — dogfood-009 spec ready).** `agent-loop.test.ts > incomplete empty-diff verdict keeps RUNNING…` flapped 2/13 host runs during the dogfood-007 review — the `waitFor` predicate (lines 251–259) gates on `wire.hits`/`currentStep`/`RUNNING` but not on the verdict being applied, so `running.lastVerdict` can read `undefined` (same sampled-vs-durable-state class as F-15, in the test harness). It sits inside a judge-executed check (dogfood-007 AC-2) so it can spuriously fail a run or harvest. Fix: gate the predicate on `report.lastVerdict !== undefined`. One-line test change, no product code. |
