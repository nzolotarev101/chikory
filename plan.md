...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)**; **P2 underway**. Eighteen dogfood campaigns reached first-attempt SUCCESS (dogfood-017 the lone FAILED — a true-positive judge ESCALATE). **dogfood-019 (`run-d836635b`) delivered WP-221 Slice A** (pure trigger half) — the WP-217 judge trigger is now a pure unit-tested `isCompletionMilestone(record)` that ORs in `claimsComplete`; functional delivery exact, behavior preserved, harvested IDENTICAL (`docs/reports/dogfood-019.md`). **But human review found F-29**: the new test's fixtures violate the `ArtifactRef` contract (7 real `TS2353` errors) yet shipped green — `typecheck` covers only `src/`, Vitest skips type-checking → **WP-230** (extend typecheck to `test/**`, fix fixtures). F-11 recurred at 25.2 % (eighteenth data point); step 1 hit a campaign-high 921k input tokens. Next: dogfood-020 / WP-230 (harden the typecheck gate before more test-bearing slices land), then WP-221 Slice B (adapters populate `claimsComplete` — retires the F-11 probe step), WP-228, WP-218 token gate. · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing. ~~WP-217~~, ~~WP-218 slice 1~~, ~~WP-220~~, ~~WP-222 slice 1~~, ~~WP-223 initial fix~~, ~~WP-224~~, ~~WP-225~~, ~~WP-209 trace slices~~, ~~WP-208 pure delivery slices~~, ~~WP-219 S1 contracts~~, ~~WP-219 `readyNodes`~~, ~~WP-219 `hasDependencyCycle`~~, ~~WP-226~~, ~~WP-227~~ (hand-landed `26b9964`), ~~WP-229~~ (dogfood-018 `run-59115f35`, F-27 closed), and ~~WP-221 Slice A~~ (pure trigger half, dogfood-019 `run-d836635b`) are delivered. **Next: WP-230** (dogfood-020 ready — extend the typecheck gate to cover `test/**` and fix the dogfood-019 fixtures; dogfood-019 F-29 proved the gate type-checks only `src/`, so contract-violating test fixtures ship green; harden the floor before more test-bearing slices land), then WP-221 Slice B (adapters populate `claimsComplete` — finally retires the F-11 probe step), WP-228 launch baseline-satisfied precheck (dogfood-017 F-25), WP-218 token-budget enforcement, and the remaining WP-219 planner/meta-judge/chain-executor slices. Rationale: `docs/reports/dogfood-002.md` through `docs/reports/dogfood-019.md`.
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Slice A done** (dogfood-019 `run-d836635b`, harvested + staged on `main` pending commit — `docs/reports/dogfood-019.md`): `agent-loop.ts:211`'s inline `completionMilestone` is now the pure exported `isCompletionMilestone(record)` (`src/workflow/judge-trigger.ts`) returning `status === "SUCCESS" && (diffRef.bytes === 0 || claimsComplete === true)`, unit-tested in isolation (six cases); behavior unchanged on today's runs (nothing sets `claimsComplete` yet). Eighteen probe data points span 5.4%-35.1%; dogfood-019 paid 25.2%. **Slice B (next after WP-230):** adapters populate `claimsComplete` from the executor's terminal output so the *productive* step is judged directly and the F-11 probe step finally disappears. NB: dogfood-019's test fixtures violate `ArtifactRef` (F-29/WP-230) — fix with WP-230. |
...
| WP-230 | Typecheck gate covers `test/**` | 🟡 | **Next up (dogfood-019 F-29; dogfood-020 ready).** `typecheck` (`tsc --noEmit`, `tsconfig.json include: ["src/**/*"]`) never type-checks test files, and Vitest transpiles without type-checking — so dogfood-019's `judge-trigger.test.ts` shipped fixtures that violate `ArtifactRef` (`{uri,sha256,bytes}` vs `{id,kind,bytes,summary}`; **7 real `TS2353` errors** when compiled) with every signal green (AC-3 clean, judge PROCEED). Add a test-inclusive typecheck (a `tsconfig.test.json` without the `src` `rootDir` constraint, or project refs) wired into a `typecheck` AC + CI, and fix the dogfood-019 fixtures to valid `ArtifactRef` values. Hardens the cheap gate the judge sits on top of. Mechanical, no contracts change. |
