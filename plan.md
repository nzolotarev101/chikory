...
**Status**: Phase 0 complete (2026-06-10); **Phase 1 complete (2026-06-11)** — all lanes M1–M5 done, dogfood-001 SUCCESS (`docs/reports/dogfood-001.md`) · **P2 underway**: WP-201 slice 1 landed via dogfood-002 (`docs/reports/dogfood-002.md`); WP-217 landed via dogfood-003 (`docs/reports/dogfood-003.md`, run `run-b2f3504d`, commit `ef4b16f`), which queued WP-221; WP-218 slice 1 delivered via dogfood-004 (`docs/reports/dogfood-004.md`, run `run-9edbcd28`, landed `2a4dd21`), which queued WP-222/WP-223; WP-220 delivered via dogfood-005 (`docs/reports/dogfood-005.md`, run `run-34926e85`, commit pending review on `wp-220-chikory-land`) — first fully priced campaign ($2.14/$5.00) — which queued WP-224; WP-222 slice 1 delivered via dogfood-006 (`docs/reports/dogfood-006.md`, run `run-559ea904`, diff verified, landed `18fae43`) — sixth first-attempt SUCCESS, first campaign with **zero new friction**; WP-223 delivered via dogfood-007 (`docs/reports/dogfood-007.md`, run `run-22b337a9`, diff verified, commit pending review on `main`) — seventh first-attempt SUCCESS, closed F-15 (3× clean full-suite post-fix) and confirmed F-14 closure (zero shim noise in executor transcript), queued WP-225 (F-19); WP-224 delivered via dogfood-008 (`docs/reports/dogfood-008.md`, run `run-86c4b628`, diff verified, commit pending review on `main`) — eighth first-attempt SUCCESS, **second zero-new-friction campaign**, closed F-17/F-18 (`land --verify` + git-stderr capture), F-11 probe tax hit a new record 25.4 % cost share; WP-225 delivered via dogfood-009 (`docs/reports/dogfood-009.md`, run `run-841bc838`, diff verified, commit pending review on `main`) — ninth first-attempt SUCCESS, **third zero-new-friction campaign**, **closed F-19** (de-flaked the WP-217 milestone test, 8/8 host runs) and F-11 probe tax hit a new record *low* 5.8 % cost share (spread now 5.8 %–25.4 %); WP-209 slice 1 delivered via dogfood-010 (`docs/reports/dogfood-010.md`, run `run-c9df353b`, diff verified, commit pending review on `main`) — tenth first-attempt SUCCESS, **fourth zero-new-friction campaign**, added the issues-found:changes-made footer metric (SE-3 concrete half), F-11 probe tax landed mid-spread 16.1 %; WP-209 slice 2 delivered via dogfood-011 (`docs/reports/dogfood-011.md`, run `run-59e0166c`, diff verified, commit pending review on `main`) — eleventh first-attempt SUCCESS, **fifth zero-new-friction campaign**, added the components-over-time timeline footer line (SE-3 temporal half — both SE-3 footer halves now render), F-11 probe tax hit a new record *high* 34.3 % cost share (spread now 5.8 %–34.3 %); WP-208 slice 1 delivered via dogfood-012 (`docs/reports/dogfood-012.md`, run `run-ea31f96c`, diff verified, harvested uncommitted on `main`) — twelfth first-attempt SUCCESS, **sixth zero-new-friction campaign**, added the pure `notificationsFor` derivation (JournalEntry[] + NotificationPolicy → ordered Notification[]; delivery + call-site deferred), F-11 probe tax 25.1 % (within spread, no record); WP-208 slice 2 delivered via dogfood-013 (`docs/reports/dogfood-013.md`, run `run-048635b1`, diff verified, harvested uncommitted on `main`) — thirteenth first-attempt SUCCESS, **seventh zero-new-friction campaign**, added the pure `slackPayloadFor` formatter (Notification → Slack `{ text }` with trigger emoji prefix; webhook POST + call-site deferred), F-11 probe tax hit a new record *high* 35.1 % cost share (spread now 5.8 %–35.1 %, set from below by the cheapest productive step yet); WP-208 slice 3 pure half delivered via dogfood-014 (`docs/reports/dogfood-014.md`, run `run-6b8f648c`, landed `3e1336f`) — fourteenth first-attempt SUCCESS, added the pure `desktopPayloadFor` formatter (Notification → `{ title, body }`), the **first run to modify an existing file** (additive), which surfaced **F-20** (harvest silently dropped modified files + falsely reported success — fixed inline, commit `0c20694`, + new `harvest-audit` confirming no past silent losses); **then the contract wall fell**: WP-219 ADR-005 accepted + slice-1 contracts landed by hand (`d56f35a` — Plan/chain types + claimsComplete/budgetTokens, TS 77 + py 47 conformance green), unblocking WP-221/WP-218-s2 and the dogfoodable chain implementation slices; WP-219 S3 pure half (`readyNodes`) delivered via dogfood-015 (`docs/reports/dogfood-015.md`, run `run-56d95ffc`, committed `40ada45`) — fifteenth first-attempt SUCCESS, **first slice to consume the ADR-005 contracts** (the run's own AC-2 kept the 77-test conformance suite green), cheapest campaign yet ($0.39); code staged and committed post-run (surfaced **F-21** → WP-226: harvest stages what it applies, now closed). next: dogfood-016 (WP-219 S3 pure precondition — `hasDependencyCycle` plan validation, the executor's acyclic guard) · **Plan date**: 2026-06-09 · **Stage 1 deadline (per spec §10)**: ~2026-09-07 (90 days)
...
**Queue order**: dogfood findings outrank the original listing — ~~WP-217~~ (done), ~~WP-218 slice 1~~ (done, `2a4dd21`), ~~WP-220~~ (done, commit pending review), ~~WP-222 slice 1~~ (done, `18fae43`), ~~WP-223~~ (done, commit pending review), ~~WP-224~~ (done, commit pending review), ~~WP-225~~ (done, dogfood-009, commit pending review), ~~WP-209 slice 1~~ (done, dogfood-010, commit pending review), ~~WP-209 slice 2~~ (done, dogfood-011, commit pending review — both SE-3 footer halves now render), ~~WP-208 slice 1~~ (done, dogfood-012, harvested uncommitted on `main` — pure `notificationsFor` derivation), ~~WP-208 slice 2~~ (done, dogfood-013 — pure `slackPayloadFor` formatter), ~~WP-208 slice 3 pure half~~ (done, dogfood-014, landed `3e1336f` — pure `desktopPayloadFor`; the pure delivery layer is now complete), ~~WP-219 ADR-005 + slice-1 contracts~~ (done by hand, landed `d56f35a` — the contract wall is down), ~~WP-219 S3 pure half `readyNodes`~~ (done, dogfood-015 run `run-56d95ffc` — landed `40ada45`). WP-221 + WP-218 slice 2 contract surface landed with the d56f35a PR (behavior pending). The next dogfoodable slice is dogfood-016 (WP-219 S3 pure precondition — `hasDependencyCycle(plan)`, the executor's acyclic guard); the remaining WP-219 implementation slices (S2 planner, S2b plan-judge, S3 chain executor non-pure, S4 handoff, S5 suspend/resume, S6 chain trace 🟢) code against frozen contracts and are dogfoodable. ~~WP-226 (F-21, dogfood-015)~~ (done): harvest stages applied files. **After dogfood-014 the pure-renderer vein is exhausted** — the remaining §6 queue (WP-208 slice 3, WP-221, WP-218 slice 2, WP-219 ADR) is all contract/ADR-gated, hand-done first per TASK-PROTOCOL §4. Rationale: `docs/reports/dogfood-002.md` (F-8/F-9/F-10) + `docs/reports/dogfood-003.md` (F-11/F-12/F-13) + `docs/reports/dogfood-004.md` (F-14/F-15/F-16) + `docs/reports/dogfood-005.md` (F-17/F-18) + `docs/reports/dogfood-006.md` (no new F-n — recurrences only) + `docs/reports/dogfood-007.md` (F-19) + `docs/reports/dogfood-008.md` (no new F-n — recurrences only) + `docs/reports/dogfood-009.md` (no new F-n — F-19 closed, recurrences only) + `docs/reports/dogfood-010.md` (no new F-n — recurrences only, F-11 mid-spread 16.1 %) + `docs/reports/dogfood-011.md` (no new F-n — recurrences only, F-11 record-high 34.3 %) + `docs/reports/dogfood-012.md` (no new F-n — recurrences only, F-11 within-spread 25.1 %) + `docs/reports/dogfood-013.md` (no new F-n — recurrences only, F-11 new record-high 35.1 %).
...
| WP-218 | Token-denominated budget + honest $0 metering | 🟡 | **Slice 1 done** (dogfood-004 run `run-9edbcd28`, landed `2a4dd21` — note: commit also carries the dogfood-004 review docs, an F-13 impurity `chikory land` now prevents; meter proven live on dogfood-005 run `run-34926e85`, $2.14 metered): pricing table prices `gpt-5.5`/`gpt-5.5-mini`/`gemini-3.1-pro-preview`/`gemini-3.1-flash` (version `2026-06-12`); trace flags `UNPRICED` steps and warns `⚠ cost meter blind` on the run header when `costEstimated` ∧ cost=$0 ∧ tokens>0. Remaining slice: `budget_tokens` cap in TaskSpec enforced by the pre-step gate — contracts change, architect-reviewed, rides with WP-221. Makes CG-2 real on subscription/zero-secrets runs. **Contract landed** via the WP-219 ADR-005 slice-1 contracts PR — `TaskSpec.budgetTokens?: number` now in `types.ts`/`schemas.ts`/`CONTRACTS.md`/`sdk-py`. Remaining: the pre-step gate enforces it. |
| WP-219 | Goal decomposition & run chaining (ADR-005) | 🔴 | **ADR-005 Accepted + slice 1 (contracts) landed** (`docs/spec/ADR-005-goal-decomposition-run-chaining.md`). The objective gap (dogfood-002 F-10a): today a goal bigger than 1–3 steps is sliced by a human; nothing plans *across* runs. Decision: a plan is a tree of ordinary judge-gated TaskSpec runs; chaining is a layer **above** the run loop (NF-1). Forks resolved: dedicated `planner/` component (D1); the plan is judged by a meta-judge, plan-judge ≠ planner family (D2); static decompose + halt-replan (D3); chain-level store (D4). **Slice 1 = the contracts PR (this change)**: `Plan`/`PlanNode`/`PlanVerdict`/`ChainLink`/`ChainRecord` + `ChainStatus`/`PlanVerdictKind` + 4 chain-scope JIF kinds, landed across `types.ts`/`schemas.ts`/`CONTRACTS.md`/fixtures/`sdk-py` (TS 77 + py 47 conformance green). **S3 pure half `readyNodes(plan, completed)` delivered** (dogfood-015 run `run-56d95ffc`, harvested pending commit — `docs/reports/dogfood-015.md`): the chain executor's dependency-resolution core (in plan order, nodes not done whose deps are all done) in `src/chain/sequencing.ts`; first consumer of the ADR-005 contracts. Remaining slices (S2 planner, S2b plan-judge, S3 chain executor non-pure, S4 context handoff via WP-203/202, S5 WP-206 suspend/resume, S6 chain trace renderer 🟢; next pure: `hasDependencyCycle` acyclic guard, dogfood-016) fall out of the ADR. Prereq for the P2 exit gate (a 24h run is a chain, not a step loop). |
...
| WP-221 | Explicit `claimsComplete` completion signal | 🟡 | **Queued (dogfood-003 F-11).** WP-217's empty-diff inference still costs one full probe step (~155–245k input tokens/slice: 155k in run-2899005b, 211k in run-b2f3504d, 158k in run-9edbcd28, 245k = **$0.33, 15.3% of run cost** in run-34926e85 — the first priced probe; 156k = $0.21, 14.8% in run-559ea904; 136k = $0.19, **18.9%** in run-22b337a9; 252k = **25.4%** in run-86c4b628; 34k = $0.05, **5.8%** in run-841bc838, the record low; 144k = $0.196, **16.1%** in run-c9df353b; 244k = $0.32, **34.3%** in run-59e0166c, the prior record high; 212k = $0.285, **25.1%** in run-ea31f96c; 220k = $0.297, **35.1%** in run-048635b1, the **new record high** (set from below — the probe was mid-range but the productive step it follows was the cheapest yet, $0.51) — spread now **5.8 %–35.1 %** over twelve data points, tracking executor discretion on suite re-run and inversely the size of the productive step it's measured against) before the trigger can fire. Note: the milestone trigger has coincided with the cadence tick in every campaign since WP-217 landed (nine straight) — it has yet to fire off-cadence in production; WP-221 is what makes it pay off. Add `claimsComplete: boolean` to `StepRecord` (contracts PR, architect-reviewed — ride the next contracts change), populate from executor adapters' final-summary signal, OR it into the WP-217 trigger so the *productive* step is judged directly. **Contract landed** via the WP-219 ADR-005 slice-1 contracts PR — `StepRecord.claimsComplete?: boolean` now in `types.ts`/`schemas.ts`/`CONTRACTS.md`/executors.md/`sdk-py`. Remaining: adapters populate it + the WP-217 trigger ORs it in (kills the F-11 probe step). |
...
| WP-223 | Watch renders journal transitions, never sampled state | 🟡 | ✅ **Done** (dogfood-007 run `run-22b337a9`, diff verified, commit pending review — `docs/reports/dogfood-007.md`): `followRun` consumes journal entries on every poll; SUSPENDED/AWAITING_APPROVAL lines derive from `budget_event` halt + ESCALATE verdict entries (printed exactly once); sampled-status prints and `lastStatus` deleted; exactly-once assertions added to `cli.test.ts`. F-15 closed: 3× host full-suite clean post-fix (it had flaked under load in dogfood-004's harvest and dogfood-006's review). |
