...
#
# Usage:
#   devbox run deconstruct-dogfood [<run-id>] [<branch-name>]
...
# 5. Git Commit (DOGFOODING.md §6)
echo "Staging all changes..."
git add -A

SUGGESTED_MSG=""
if [ -n "$SPEC_NAME" ]; then
  CLEAN_NAME=$(echo "$SPEC_NAME" | sed -E 's/^dogfood-[0-9]{3}-//')
  SUGGESTED_MSG="feat: $CLEAN_NAME (run $RUN_ID)"
else
  SUGGESTED_MSG="feat: harvest dogfood run $RUN_ID"
fi

echo "Committing changes with pre-populated message template..."
git commit -e -m "$SUGGESTED_MSG"

# 6. Output guidance on PR review
echo ""
echo "=========================================================="
echo "Successfully applied and committed changes from run $RUN_ID!"
echo "=========================================================="
echo "Next steps:"
echo "1. Push your branch and propose a PR:"
echo "   git push -u origin $BRANCH_NAME"
echo ""
echo "   PR description template:"
echo "   ---"
echo "   Chikory Dogfooding Run: $RUN_ID"
echo "   "
echo "   Verification checks executed:"
echo "   - devbox run lint"
echo "   - devbox run typecheck"
echo "   - devbox run test"
echo "   ---"
echo ""
echo "2. Run the mandatory post-run review using the Antigravity skill:"
