...
- **Landed commit:** `774e130` (`feat: implement pairwise and G-Eval scoring modes in the judge SDK`) — an **auto-commit hook** bundled the 3 delivery files byte-IDENTICAL to the run workspace (`dogfood-verify §5` confirmed IDENTICAL before the commit) **together with this report + the dogfood-058 living-doc edits**, with a generic message and **no run-id stamp** → `git log --grep <run-id>` can't resolve it (the F-51/WP-249 harvest-hygiene gap recurs, now via an auto-commit). At review time `dogfood-verify §6` read "no landed commit" because the commit fired after the pack ran.
...
- Operator started the cli-judge-proxy shim, committed the dogfood-057 delivery so the tree was clean, launched once (no seam env), watched to terminal. **An auto-commit hook then landed the delivery + this report + the doc edits as `774e130` mid-review** — a generic message, no run-id stamp, delivery bundled with docs. This is the **F-51/WP-249 harvest-hygiene gap recurring through automation** (run-diff not separated from operator/doc edits; no `Ref: run-id:` line) — no new F-number, it reinforces WP-249's priority. The dogfood-059 readiness edits (spec + NEXT pointers) remain uncommitted for review.
