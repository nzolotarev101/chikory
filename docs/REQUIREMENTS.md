...
| OB-5 | Trajectory-level forensics, whole path debuggable after the fact | WP-122 (journal), WP-142 (renderer), WP-231 (dogfood landing-scope audit), WP-403 (web) | P1→P4 | in-progress (P1 journal + renderer done; **WP-231 queued from dogfood-031 F-31** to make "what ran" vs "what landed" mechanically auditable by comparing the run workspace diff with the landed commit and flagging extra/missing/different paths; web browser P4) |
