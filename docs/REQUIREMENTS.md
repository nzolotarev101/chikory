...
| IF-2 | Thin Python SDK | WP-201 | P2 | in-progress (slice 1 — contracts + shared conformance suite — landed `eb5c57e` via dogfood-002; pure compaction digest-prompt parity done via dogfood-030 `run-1a97e2ca`: `DIGEST_SYSTEM_PROMPT` + `build_digest_messages(to_digest) -> list[Message]` in `packages/sdk-py/src/chikory/compaction_prompt.py`, re-exported from `chikory`, 4 focused pytest cases, no contract/runtime wiring change) |
