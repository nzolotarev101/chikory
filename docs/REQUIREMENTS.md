...
| JD-3 | Gates next action: halt/rollback/branch/escalate before bad change lands | WP-132 (halt/rollback/escalate); branch verdict WP-205 | P1→P2 | in-progress (halt/rollback/escalate done; branch verdict P2; WP-205 pure surface — parser dogfood-033 + branch-name helper dogfood-034 — done before branch side effects) |
...
| OB-5 | Trajectory-level forensics, whole path debuggable after the fact | WP-122 (journal), WP-142 (renderer), WP-231 (dogfood landing-scope audit), WP-403 (web) | P1→P4 | in-progress (P1 journal + renderer done; **WP-231 done via dogfood-032 `run-4f1fbe0a`, landed `ded282b`**: `dogfood-verify` now has a landed-scope section and `scripts/dogfood-landed-scope.sh` compares the run workspace diff with the landed commit, flagging extra/missing/different paths; web browser P4) |
...
| SE-2 | Window-fit reasoning for implementation and judging passes | WP-207 | P2 | in-progress — WP-207 pure helper done via dogfood-031 (`decideContextWindowPacing`, `docs/reports/dogfood-031.md`); remaining work is using that decision in planner/runner cadence and journaling. |
