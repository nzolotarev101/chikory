...
| IF-4 | CLI surface | WP-141, WP-142; WP-220 (`chikory land`, delivered via dogfood-005 `run-34926e85`, commit pending review); WP-223 (watch transition fidelity — delivered via dogfood-007 `run-22b337a9`, commit pending review; F-15 closed, three clean full-suite runs post-fix); WP-224 (`land --verify` + git-stderr seam, dogfood-005 F-17/F-18 — dogfood-008 spec ready) | P1→P2 | reopened (watch fidelity fixed by WP-223; row stays open only for WP-224 — `land` commits unverified and leaves `dist/` stale, dogfood-005 F-17) |
