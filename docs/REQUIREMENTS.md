...
| JD-5 | Structurally diversified: different model family / prompt regime / memory than executor | WP-133, ADR-002; WP-222 slice 1 (executor env allowlist — delivered via dogfood-006 `run-559ea904`, commit pending review) | P1→P2 | done (declared-family check); seam hardened — `scrubExecutorEnv` gives executor children only their own family key; F-14 closure confirmed by the next campaign's clean executor steps; TaskSpec pass-through opt-in rides WP-221's contracts PR |
...
| IF-4 | CLI surface | WP-141, WP-142; WP-220 (`chikory land`, delivered via dogfood-005 `run-34926e85`, commit pending review); WP-223 (watch transition fidelity, dogfood-004 F-15); WP-224 (`land --verify` + git-stderr seam, dogfood-005 F-17/F-18) | P1→P2 | reopened (`run --watch` can miss a SUSPENDED→RUNNING transition between polls — dogfood-004 F-15, fix WP-223; `land` commits unverified and leaves `dist/` stale — dogfood-005 F-17, fix WP-224) |
