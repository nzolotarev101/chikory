...
| DX-5 | Branching of execution paths as first-class op | WP-205 | P2 | in-progress — pure CLI target parser done via dogfood-033 (`run-41dd7c98`, `docs/reports/dogfood-033.md`): `parseBranchTarget(input)` accepts `<run-id>@<positive-integer-step>` and `<run-id>@base`, returns a canonical `checkpointId`, and rejects malformed targets. Branch command dispatch, journal fork, and git worktree side effects remain. |
| DX-6 | Rollback as first-class op | WP-132 (judge-triggered), WP-205 (manual) | P1→P2 | in-progress (judge-triggered done; manual `chikory branch` P2; WP-205 parser slice done in dogfood-033, branch/worktree side effects remain) |
...
| JD-3 | Gates next action: halt/rollback/branch/escalate before bad change lands | WP-132 (halt/rollback/escalate); branch verdict WP-205 | P1→P2 | in-progress (halt/rollback/escalate done; branch verdict P2; WP-205 parser slice done via dogfood-033 before branch side effects) |
