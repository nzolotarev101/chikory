...
| NF-6 | No LLM-layer mocks in integration tests; test code held to the same type contract as `src/` | CI policy (WP-003), CLAUDE.md. **Gap open — WP-230** (dogfood-019 F-29): `typecheck` covers only `src/**`, so contract-violating test fixtures ship green (dogfood-019's `judge-trigger.test.ts` had 7 unseen `ArtifactRef` `TS2353` errors). Extend the gate to `test/**` + fix the fixtures; next up dogfood-020. |
