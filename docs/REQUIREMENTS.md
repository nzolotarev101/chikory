...
| CM-3 | Memory Pointer Pattern: external storage + short refs in context | WP-202 | P2 | in-progress — the local-FS content-addressed `ArtifactStore` (`src/artifacts/local.ts`: `put`/`get`/`excerpt`, sha256 ids stable across resume/branch) is landed; the pure Memory Pointer **workflow** half (decide when a tool output is large enough to store externally + render the short context reference) is queued as **dogfood-028** (`shouldPointerize(bytes, policy)` + `formatPointerReference(ref)` in a new `src/runner/memory-pointer.ts`, the analog of `buildVerdict`/`evaluateBaselinePrecheck`; local policy type, imports only the frozen `ArtifactRef`, no `types.ts` change). The non-pure half (intercept a tool output → `shouldPointerize` → `store.put` → inject `formatPointerReference`, else inline) is the hand-design follow-up (TASK-PROTOCOL §4) and the first step toward unblocking WP-203 S2 digest wiring. |
