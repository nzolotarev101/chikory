...
🔴 **F-180 — the campaign's highest-value seam:** the judge NAMED F-181 and F-182 pre-land (`design_serves_overall_goal ✗`) and the run sealed 🟢 SUCCESS anyway. `verdict.ts:131-149` — a rubric-only failure falls to a fallthrough that ALSO returns PROCEED, so it changes only the rationale STRING, and the runner seals on all-criteria-pass. WP-311 design judging is inert at the seal altitude → **WP-537**.
...
| The judge's rubric flags a real design defect and the run seals 🟢 SUCCESS anyway | **Known and unfixed — F-180 → WP-537 (dogfood-113).** `packages/sdk-ts/src/judge/verdict.ts:131-149`: with every AC passing and one rubric item failing, control falls to a fallthrough branch that ALSO returns `PROCEED`, so the rubric failure changes only the rationale text; the runner then seals run-level SUCCESS on all-criteria-pass. **Until WP-537 lands, ALWAYS read the `rubric:` block in `dogfood-verify.sh` §2 — a `design_serves_overall_goal ✗` on the sealing step is a real pre-land catch the control plane did not act on.** On dogfood-113 it named two defects the review independently confirmed. |
