...
Latest: dogfood-091 — **WP-251 LIVE COMPACTION-FOLD OBSERVATION (the ⑧ P2-exit context-rot axis), FALSE-FAILED** (`run-7fca16bc-8296-4b24-a9bc-b7e7641dd533`,
`docs/reports/dogfood-091.md`). 🔴 **FAILED · 4 steps · $3.32/$80 · 12m 43s · NOT harvested.** The 4-part product delivery (net-new pure `describeCompactionPressure`
pressure-join reducer + `chikory trace` render + live Temporal fold test) was CORRECT and full-suite-green in-workspace, but the run false-FAILed on 🔴 **F-119** —
AC-2 `test "$(grep -rc … src/)" -ge 2` is UNSATISFIABLE (`grep -rc <dir>` is multiline → `integer expression expected` → exit 2 on EVERY judge pass → the "AC failed
3+ consecutive → HALT" budget-waste guard guillotined a green delivery). Separately 🟡 **F-120** — the real run NEVER FOLDED (`peak window 1% · compact 0`):
`CHIKORY_CONTEXT_WINDOW_TOKENS=1200000` was sized for codex's internal ≈400k–900k tokens, but the pacing window tracks Chikory's ASSEMBLED-CONTEXT tokens (~3k
projected) → never crossed the compact band. **No rung advanced (ledger rung 0); WP-251 STILL un-live-proven.** F-119 lint LANDED in `dogfood-progression.sh`
(⛔s any AC piping `grep -c`/`grep -rc` into an arithmetic `test`; see §7). Progression **✅ PROGRESSING** unchanged (trailing-3 rung max 4 held).
Post-review (2026-07-08) a THIRD 091 defect surfaced: 🔴 **F-121** — the 091 journal (`runs.task_json` has no `debug` key; every pacing entry's denominator is the
400k gpt-5.5 default) proves the "armed" window env NEVER reached the workflow: the challenge silently no-op'd and nothing warned. Launch preflight now guarantees
hypothesis+challenge at $0 (see §4): spec-named `CHIKORY_*` envs are a launch contract, the window scale is sanity-checked, and every non-suite AC check is DRY-RUN
against HEAD (a BROKEN check or zero RED-on-HEAD ACs refuses the launch).
**NEXT: dogfood-092 — RE-RUN WP-251** with (1) AC-2 → the occurrence-count idiom `grep -roh … | wc -l`, (2) `CHIKORY_CONTEXT_WINDOW_TOKENS=2000` — journal-derived
(091 projected 2109→3027; compact threshold 0.8×2000=1600 < proj₀ 2109 folds from step 1; ~5k, threshold 4000, would never fold). See §7, §1.5, §1.4, §3.
(dogfood-090 WP-105 detail → PLAN-HISTORY.)
