...
| A chain runs end-to-end but a later node seals FAILED/HALT with `changes made 0` and `criterion … failed 3+ consecutive verdicts → HALT`, even though an earlier node it depends on SUCCEEDED | The dependent node ran against a **fresh clone of HEAD without the predecessor's diff** — chain S4 context handoff is **not yet implemented** (dogfood-041 attempt 5, **F-37**; `chain-loop.ts:88` spawns each node from `template.repos`/HEAD with no `parentRunId`, `node-spec.ts:68`). The node had nothing to build on, produced an empty diff, and the judge correctly HALTed it. **Until WP-237 (S4 handoff) lands, decompose goals into single-node or fully INDEPENDENT (parallel) nodes only — a node must not depend on another node's *code changes*.** A genuinely dependent multi-node chain (the compounding-error case) cannot go green yet. This is expected, not a regression. |
