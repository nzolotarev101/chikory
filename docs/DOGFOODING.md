...
**✅ WP-540 (base-green verification) IS DONE — dogfood-118, `run-ee8491db-a082-4eb3-97ee-a9915256b401`, SUCCESS 1 step $0.0487/$30, 3m 18s, executor `gemini-cli`, judge `openai-compat`, PROCEED 4/4, harvest byte-IDENTICAL (7 files, +301/−54), 102 harness / 1007 sdk-ts / 84 py green.** The harness now clones `repo.url`@`repo.ref` into `os.tmpdir()/base-verify-*`, verifies THERE under the WP-538 provisioning decision, and deletes it in `finally`; the adapter workspace still starts empty (trap A) and verification precedes `adapter.run` (trap B). The guessing resolver is gone; one declared field `base_verification_command` replaces it. **F-198/F-199/F-200 CLOSED.**
🔴 **F-203 — RE-MEASURE A `blocked_reason` BEFORE SPENDING A RUN ON ITS UNBLOCK (new standing rule, §7).** `brownfield-002` has carried `status: blocked` since 2026-07-23 on F-163's claim that its base is systemically red under Node 22. Re-measured on a clean clone of the pinned ref `a061eaa1`: `pnpm install --frozen-lockfile` 5.2s + `pnpm test` → **153/153 files, 1128/1128 tests passed, 13.13s, ambient node v22.22.3**; `decideTargetNode(">=24")` also resolves `provision` → nodejs-24.15.0. WP-534/538/540 — four runs, three at `rung=0` — were built to unblock a task that was not blocked. Cost of the check: ~20 seconds.
🔴 **F-202 — THE VERDICT HAS NO CONSUMER.** `suite.ts:197` writes `baseVerification` onto the per-task record; zero readers, no `SuiteSummary` field (`results.ts:26-45`), I-SR/D-SR computed over every task regardless. A benchmark can still publish a number whose base was never proven green. → **WP-541 / dogfood-119.**
⚠️ **THE AC LESSON, THIRD STATEMENT (§3.4):** F-187 — enumerate every INPUT family. F-196 — pin the OUTPUT's CONSEQUENCE. F-198 — probe the ALTITUDE the defect lives at (module vs call site). F-202 adds: **ask who CONSUMES the output, one hop further out than the spec's own scope.** dogfood-117's defect was the call site; dogfood-118's is the consumer; both runs passed every genuine, behavioral AC they were given.
🟡 **F-204 — a vacuous cleanup assertion (my spec, not the delivery).** AC-1 scanned the fixture's own temp root for prefix `chikory-base-`, which no implementation uses (the delivery writes `base-verify-*` to `os.tmpdir()`), so it would have passed over a delivery that leaked every checkout. Cleanup is in fact correct — pinned by the executor's `suite.test.ts:193` and hand-checked (0 leftovers). An oracle-owning AC must assert against a path the implementation was TOLD to use, or assert on the directory it was given. → track-B AC hygiene.
ℹ️ No new id: **F-205** (base checkout re-implements `engine.ts:210-217`'s temp clone as a full `git clone` + `checkout` with hardcoded 30s/15s timeouts — measured harmless: gitify 2.08s/27MB, zod 1.97s/40MB, both pinned refs reachable; fails only for a ref unreachable from any branch) · **F-206** (nothing installs deps between checkout and verification — declarations must fold install in; measured 18.3s against the 120s `DEFAULT_CHECK_TIMEOUT_MS`) · **F-167/F-9** ($0.0000 over 5,183 metered executor tokens) · **F-190** (one agent session = one step, so `judge.cadence: 2` never fired twice) · judge cost share **100%** vs `maxCostShare: 0.5`, structural for a 1-step gemini run.
Progression gate = ⛔ **STALLED** (3rd consecutive `rung=0`; trailing-3 max steps 2 vs 4). Next headline = **dogfood-119 / WP-541 (an unverified base invalidates the score, + the `brownfield-002` flip)** — rung-4 itself still cannot run: corpus is 2 (3 after the flip) of the 5 it needs, and the baseline arm (WP-304) has never run.
