...
| `chikory chain --watch` goes silent while a child is `AWAITING_APPROVAL` | **F-42 → WP-241** (dogfood-042): the child ESCALATE is not projected into the ChainJournal/watch surface. Inspect the child journal, then approve/resume its exact run id; the durable parent will continue when a worker reattaches. WP-241 will add chain-visible reason/status and chain-level approval/resume UX. |
