...
| The judge proxy "is already running" but chain/judge LLM calls get `transport error: fetch failed` | `dogfood.sh` skips proxy startup when :8787 is in use ("Assuming proxy is already running") **without health-checking it** (dogfood-041 **F-34**); a stale/dead/half-crashed listener — or one whose backend CLI is failing — presents as an in-use port. **Fix now:** kill whatever holds the port and start a fresh proxy — `lsof -ti:8787 | xargs kill; node scripts/cli-judge-proxy.mjs 8787 gemini &` — then probe it (`curl -s http://127.0.0.1:8787` should respond) before launching. WP-234 will make `dogfood.sh` do this probe automatically. |
