...
**NEXT: dogfood-092 — RE-RUN WP-251** with (1) AC-2 → the occurrence-count idiom `grep -roh … | wc -l`, (2) `CHIKORY_CONTEXT_WINDOW_TOKENS` sized to the
assembled-context scale (~5k, above compact / below park) so a real `chikory run` actually FOLDS. See §7, §1.5, §1.4, §3. (dogfood-090 WP-105 detail → PLAN-HISTORY.)
