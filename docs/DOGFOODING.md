...
judge's evidence. Next: dogfood-032 takes **WP-231 landing-scope audit** so
`dogfood-verify` mechanically compares "what ran" to "what landed".
...
| The run diff is exact, but the landed commit contains extra files outside the spec | Post-run commit-scope contamination (dogfood-031 **F-31**): the judge reviewed the run workspace diff, but `HEAD` (`67eb167`) also included unrelated warning-suppression edits. Until WP-231 lands, manually compare `git -C .chikory/runs/<run-id>/workspace diff --name-status chikory-base..HEAD` with `git show --name-status <landed-commit>` before trusting the commit. WP-231 adds a dogfood verifier landing-scope audit that flags extra, missing, or different paths mechanically. |
