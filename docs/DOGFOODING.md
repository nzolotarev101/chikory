...
**Status (2026-07-27, bounded — update discipline: REPLACE this block, ≤15 lines;
displaced prose moves verbatim to [`PLAN-HISTORY.md`](PLAN-HISTORY.md); per-run detail:
`docs/reports/`; queue + course correction: `plan.md` §6/§7).**
**⛔ dogfood-115 (WP-537, `run-c19147fe-13b6-4964-834d-9d55ccd99484`) FAILED — 4 steps, $0.1576/$30, 7m 27s — and NOTHING WAS EVER GRADED. `docs/reports/dogfood-115.md`.**
🔴 **F-192 — THE EXECUTOR DID THE WORK IN THE OPERATOR'S CHECKOUT, NOT THE GRADED CLONE.** Every step diff was 0 bytes; the judge graded an empty tree, went 1/4 on ACs and HALTed after 3 stuck verdicts — while a complete, all-4-ACs-green WP-537 implementation (206 insertions) sat unversioned in the working tree, outside trace, checkpoints, rollback and harvest. Proof is mechanical, not prose: the workspace's `git status` is clean and every target file is untouched at its 14:53 prep mtime, while the source checkout's four files were written at 14:54:26–14:56:21, inside steps 1–2. The clone's `origin` points straight back at the source, and `agy` runs under `--dangerously-skip-permissions`.
⚠️ **THE GATE WAS FLAWLESS — read this run as a WIN for the thesis and a LOSS for the substrate.** Handed an empty tree and a ~600-word executor summary claiming green tsc, green eslint, 10 unit + 4 live tests and a completed 2×2×3 matrix, the judge executed the ACs ITSELF, found 3 of 4 red, refused to seal, and wrote the true state into the rubric: *"No implementation design is present in the empty diff."* The judge can only grade what the harness hands it — that is the layer that broke.
🔴 **F-193 (NEW, HAND-FIXED) — THE REVIEW TOOL ITSELF REPORTED FALSE FAILS.** `scripts/dogfood-verify.sh:67` collapsed multi-line `check:` bodies with `replace(/\s+/g," ")`, folding AC-1's `//` comment over the rest of its `node -e` script (`SyntaxError: Unexpected end of input`) and gluing AC-3's `vitest` and `node -e` lines into one argv. Both re-ran as FAIL against a delivery that PASSES. The judge's own runner preserves newlines, so only the review half was wrong. Now base64 emit/decode (`:67-73`, `:160-161`).
🟢 **HAND-FIXED THIS SITTING (F-192 detection):** `executors/workspace.ts:64-96` `sourceRepoDirtyPaths()` resolves the clone's local `origin`; `executors/step.ts:95-97,109-115,177-199` samples it before and after a step but ONLY when the diff is 0 bytes, and FAILs the step (retriable) naming the escaped paths. Zero cost on any step that produced work; disarms when `origin` is absent or non-local. Plus a named workspace boundary in `executors/prompt.ts:50-61`. **6 tests**, sdk-ts **988 passed / 23 skipped** on the landed state (982 on HEAD + these 6; 994/23 with `stash@{0}` applied, which is NOT what landed), tsc + eslint clean.
⚠️ **DETECTION ≠ PREVENTION → 🔴 WP-539 (queued).** The write is still possible: `runner/paths.ts:26-28` nests the graded clone inside the repo it clones, while the bench harness already clones to `os.tmpdir()` (F-184). A minimal probe (nested clone, same `agy` flags) wrote correctly in-workspace WITH and WITHOUT the boundary text, so the mechanism is **not pinned** — do not claim it is fixed.
🟡 **F-194:** AC-1 probed **7** field spellings for "the rubric failed" (to leave the design open) and the executor implemented all 7, 6 of them dead API. The mirror of F-187: probing one input family ships regressions, probing many ships dead surface. §3.4 guidance owed. 🟡 **F-195:** 3 pacing `compact` events with 0 folds (`pressure-steps 3 (unfolded 3)`) — probably an F-192 artifact (empty context); re-check on 116.
**WP-537 IS NOT DELIVERED — it re-runs as dogfood-116.** The unjudged delivery stays in `git stash@{0}`: landing an ungraded delivery by hand is precisely the failure WP-537 exists to abolish. It IS the reference implementation proving AC-1 GREEN; HEAD proves it RED (`FAIL: a FIRST-VERDICT seal carrying a failing rubric item must REVIEW`). AC wall-clocks vs the 120 s judge cap: AC-1 **3 s**, AC-3 **8 s**, AC-4 **9 s**.
ℹ️ Recurrences, no new id: **F-167/F-9** ($0.0000 over 26.7k tokens) · **F-176** (`toolCalls: 0` on all 4 steps made the escape harder to diagnose) · **F-190** (every step a first-verdict seal) · **F-168** (gate ⛔ message still cites the retired P2/WP-265 ladder).
Progression gate = ⛔ **STALLED** going in, and this run moved no axis — dogfood-116 re-runs the same rung-4 unblock. P3-rung-4 corpus reach still 2 runnable (`brownfield-002` `status: blocked`).
See §5 (ladders always exist; the AC-enumeration rule), §7, §8, §1.5, §3. (Earlier — dogfood-113/114, WP-534/538, F-180…F-191, bench suite `20260725-130418` → PLAN-HISTORY.md.)
