...
**✅ WP-538 (range-correct, single-plan, fail-loud node provisioning) DONE 2026-07-27 — dogfood-114, `run-fc10fe73-0095-48b9-b994-b17e1f27d51d`, SUCCESS 1 step $0.0552/$30, 3m 45s, judge PROCEED 5/5 AC + 6/6 rubric, harvested + hand-fixed (`docs/reports/dogfood-114.md`).**
🟢 The harness now picks a Node that genuinely SATISFIES each target's declared range, decides it ONCE per task, and hands the same decision to the run path and the grading path (`nodeProvisioning` on `AdapterContext`/`GradeContext`; neither consumer re-derives, both throw on `unavailable`). Live-probed at ambient v22.22.3: gitify `">=24"` → `provision …-nodejs-24.15.0/bin`; zodios + zod (no `engines`) → `ambient`. **F-181/F-182/F-183/F-184/F-185 all closed.**
🔴 **F-187 — a REGRESSION shipped under 5/5 AC + 6/6 rubric, hand-fixed in the review sitting.** An AND-range (`">=24 <26"`, `">=18 <21"`, `">=18.19.1 <19 || >=20.11.1"`) matched none of the four single-comparator regexes → classed unparseable → **silently ambient**: the exact wrong-node red the WP exists to abolish, and worse than the legacy resolver (which provisioned node 24 for `">=24 <26"`). One `parseAtom`/`parseRange` model now covers `>=`/`>`/`<=`/`<`/`=`/`^`/`~`/`N.x`/`*`/bare as OR-groups of AND-ed comparators. Also 🟡 **F-188** (absent `package.json` ≠ unreadable repo → was SKIPPING every non-Node target; `git ls-tree` probe distinguishes them) · 🟡 **F-189** (a full bare pin `"24.15.0"` was satisfied by 24.14.1; now exact). Harness **78/78** (59 on HEAD), tsc + eslint clean.
⚠️ **THE AC LESSON (new, §5):** an oracle-owned AC still only proves what it ENUMERATES. AC-1 asserted behavior against the real built module and was verified both directions — and still missed the bug, because every probe used a single-comparator range. **When the deliverable is a parser or a matcher, the AC must enumerate the INPUT FAMILIES the real world contains, not just the happy family the goal names.**
🔴 **F-180 RECURRED → WP-537 is the NEXT HEADLINE (dogfood-115).** The rubric was 6/6 ✓ this time, so the gate did not even flag F-187; and being a 1-step run, the completion review was skipped again. Two consecutive headlines sealed 🟢 SUCCESS carrying human-findable defects.
🟡 **F-190 (track-B, binding on WP-537):** `gemini-cli` collapses a whole agent session into ONE Chikory step — step 1 journaled **0 tool calls** and 4.8k/2.4k tokens for a **19,482-byte 6-file diff**. So max-horizon-survived reads 1 on every run, `judge.cadence: 2` can never fire twice, checkpoints are one commit, context-rot mitigation is never exercised, and the "make the goal big enough to need >1 step" F-180 mitigation is structurally unavailable: **every run on this arm is a first-verdict seal.**
🟡 **F-191 (NEW, hand-fixed):** the spec-format lint scanned the WHOLE spec file for prescribed-diff shape, so an oracle-owned `node -e` AC ⛔'d a purely outcome-shaped spec as PRESCRIBED — the lint was pressuring headlines away from the very AC style §3.4 now requires. `scripts/dogfood-progression.sh` now reads a `goal:`-only block (114 + 115 🟢 LOOSE, 012…017 still PRESCRIBED).
ℹ️ **F-167/F-9 recurrence:** 7,212 metered executor tokens at $0.0000 (`⚠ cost meter blind` rendered correctly). The JD-7 "judge spend is 100.0% of run cost" warning is a **denominator artifact**, not overspend — $0.0552 total, 0.2% of budget; `max_cost_share` is inert on this arm. 🟡 F-186 did NOT recur (harvest passed first try; sdk-ts 982 pass / 23 skip).
**P3-rung-4 — the mechanical wall is GONE, corpus reach still 2 runnable.** `brownfield-002` needs its `status: blocked` → `pinned` flip behind a green-base verification bench run (→ 3), then 2 more pinned tasks (WP-302) to reach ≥5, then a baseline arm (WP-304). Sequencing: **WP-537 first** — do not author a corpus we intend to PUBLISH under a gate that cannot gate.
Progression gate = ✅ **PROGRESSING** with a standing ⚠️ LADDER PACE warning (rung 3→3→3; this run is `rung=0`, mode `run`, no suite scored — a rung-4 UNBLOCK, not a climb).
See §5 (ladders always exist; the AC-enumeration rule), §7, §8, §1.5, §3. (Earlier — dogfood-113 / WP-534 / F-180…F-186 / bench suite `20260725-130418` → PLAN-HISTORY.md.)
