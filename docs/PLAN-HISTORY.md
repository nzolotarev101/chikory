...
> **NEXT HEADLINE (progression ⛔ STALLED — rung flat 4 (ladder RETIRED at rung 5, dogfood-096/P2 exit — the mechanical STALLED can't credit a retired ladder OR chain-horizon (F-145); real axis = the flat kill→resume/chain-recovery KPI, 0 across 097…103): dogfood-104 (WP-521 CHAIN HEAL-BY-DEFAULT + `chikory chain resume` FOR FAILED CHAINS — default `maxReplans`≥1 (today 0, `chain-loop.ts:70`), evidence-enriched self-contained replan, chain resume past parked-only; proven on a REAL 3-node chain whose MIDDLE node fails its first incarnation and self-recovers via the default replan — KPI journal-read; user-confirmed direction).** The P3 §7 thesis-mechanism backlog (WP-307/308/309/310/311) is now EXHAUSTED. **P3 proof ladder INTRODUCED (WP-530, §7 — every phase carries a ladder; DOGFOODING §5 gate 0):** P3-1 chain self-heal (= dogfood-104) → P3-2 operator `chikory chain resume` → P3-3/4 benchmark tasks scored → P3-5 (EXIT) published numbers + leaderboard. **dogfood-104 = P3-rung-1 ATTEMPT; its WP-521 substrate (a) heal-by-default + (b) self-contained evidence-enriched replan + the `CHIKORY_SEED_CHAIN_FAIL_NODE` seam + live regression are HAND-LANDED + COMMITTED 2026-07-15 (923 TS green).** ⚠️ **dogfood-104 RAN but did NOT climb rung-1 (`chain-83913c34`, `docs/reports/dogfood-104.md`): the chain sealed SUCCESS with 0 `node_replanned` — the force-fail seam was NOT armed at launch (F-146, no preflight guard), so no node failed and heal-by-default never fired. Delivery (per-node recovery-summary surface) landed clean, 934 TS green, but P3-rung-1 UNPROVEN → RE-RUN = dogfood-105 (seam ARMED). WP-531 (chain-dogfood preflight guard, F-146 fix) ✅ HAND-LANDED 2026-07-16 (`scripts/dogfood.sh` 1d-bis: a `--chain` heal spec is refused unless the seam is armed, own override `CHIKORY_ALLOW_UNARMED_HEAL=1`; the coarse `ALLOW_MISSING_ENV` can no longer silence it).** (WP-521 (c) operator chain-resume = P3-rung-2.)

Prior DOGFOODING.md header status block (dogfood-104 Latest), displaced verbatim:

> Latest: dogfood-104 — **WP-521 (chain heal-by-default) P3-rung-1 attempt: recovery-observability surface LANDED 🟢, chain self-heal KPI NOT EXERCISED 🔴** (`chain-83913c34-312d-4c7f-95f4-832a6a168473`, `docs/reports/dogfood-104.md`, harvested to working tree, uncommitted). Delivery 🟢: a real 3-node chain built a per-node RECOVERY-summary surface (`summarizeNodeRecovery` + `renderChainRecoverySummary` + additive guarded `trace.ts` wire), full suite **934 TS pass / 22 skipped**, tsc + eslint clean, 6 files all `packages/sdk-ts/`. **Thesis-KPI 🔴 NOT MET:** chain terminal SUCCESS with **0 `node_replanned` / 0 `chain_failed`** — the force-fail seam `CHIKORY_SEED_CHAIN_FAIL_NODE` was NOT armed at launch, so all 3 nodes sealed SUCCESS first incarnation and the heal-by-default path never fired. P3-rung-1 (chain self-heal) **UNPROVEN** → ledger `rung=0`, re-run owed. One incidental run-scope tick: node A step 2 stalled → killed by `maxSeconds` (retriable) → step 3 resumed to SUCCESS (`resumes=1`, first non-zero in the window). New friction: 🔴 **F-146** (heal-ladder chain launched with the seam UNARMED, no preflight guard caught it — $5.51/56m proved nothing on the headline axis → WP-531 track-B, HAND-FIX this sitting) · 🟡 **F-147** (cost/token telemetry $0.00/0 tokens on node A's 2 FAILED steps despite 13 tool calls) · 🟡 **F-148** (node A ~36m thrash across 2 failed steps before a 2m delivery; the maxSeconds kill→resume is the silver lining). **NEXT headline (progression ⛔ STALLED; rung-1 still owed): dogfood-105 = P3-rung-1 RE-RUN, seam ARMED (`CHIKORY_SEED_CHAIN_FAIL_NODE=B`). WP-531 (F-146 fix) ✅ LANDED 2026-07-16 — `scripts/dogfood.sh` 1d-bis refuses a `--chain` heal spec unless the seam is armed (own override `CHIKORY_ALLOW_UNARMED_HEAL=1`; `ALLOW_MISSING_ENV` can no longer silence it); dogfood-104's delivery committed (`f303ed8`). dogfood-105 is launch-ready.**
...
Prior DOGFOODING status head (dogfood-095), displaced verbatim:
...
> - **Phase:** P0 ✅ (2026-06-10) · P1 ✅ all 5 MVP exit gates (2026-06-11) · **P2 underway — BOTH ⑦ and ⑧ mechanism axes now CLOSED. ⑦ literal-overnight done (dogfood-090, 6h19m / 4 parks > token-TTL, F-116 cured live cross-worker). ⑧ LIVE compaction fold FINALLY OBSERVED (dogfood-093, `run-60a32aff`, 7 steps: `compactions 2 (pacing 2) · first pacing fold step 5`) — the ≥6-step horizon (F-122 fix) crossed `keepLastN=5` and `planCompaction` folded twice. Every P2-exit MECHANISM (soak endurance, window-park, live compaction, durable OTel spans, run-level self-heal) is live AND the combined rehearsal (auto-calibrated pacing fold + deterministic kill→resume) PASSED end-to-end on a real horizon (dogfood-095, `run-7746e7fa`); the only remaining P2 item is the launch-gated 24h+ brownfield run that COMBINES them.** · P3 NOT started — Stage-1 90-day clock says published numbers due ~2026-09-08.

> - **Last 3 runs (091/092 detail → PLAN-HISTORY.md 2026-07-09):** **dogfood-093 — WP-251 ⑧ re-run, THE FOLD (`run-60a32aff`, 🟢 SUCCESS · 7 steps, $4.53/$80, 20m52s, uncommitted):** the F-122 fix — `min_durable_steps: 6` + 7 chunks (SAME ~2000 window) — pushed resident summaries past `keepLastN=5` and **`planCompaction` FOLDED TWICE** (`compactions 2 (pacing 2) · first pacing fold step 5`). Net-new increment: `pressureFoldGapWarning` (F-123 loud-on-silent) + additive `firstPacingFoldStep`; 790 TS green, 0 rollbacks. **WP-251 → 🟢 LIVE-PROVEN; ⑧ context-rot axis CLOSED** (arc 053/091/092/093). 🟡 **F-124** — compaction entry lacks `stepIndex` (inferred first-fold step; track-B under WP-251). · **dogfood-094 — P2 EXIT-GATE REHEARSAL (reactive-signals greenfield, user-directed) — delivery 🟢, rehearsal 🔴 0/2 axes (`run-b319713b`, 🟢 SUCCESS · 11 steps, $1.95/$40, 13m10s):** Chikory autonomously built a real correct fine-grained reactive-signals lib (11 increments, 21/21 `node --test` green, 6 primitives incl. glitch-free diamond). **But the Thesis-KPI missed BOTH axes:** window `4000` was too big for this small greenfield (proj 1.6–2.6k = 40–65%) → pacing stayed `continue` all 11 steps (`peak window 65% · compact 0`), the 3 folds were `trigger:"count"` (`compactions 3 (pacing 0)`) NOT pacing; and the manual kill→resume never fired (`resumes 0`). 🔴 **F-125** — seam-window sizing is workload-dependent, NO calibration (4th sizing miss: 053-overshoot/091-undershoot/092-too-short/094-too-big) → WP: pre-flight window auto-calibration off step-1 assembled tokens. 🟡 **F-126** — stale-spec precheck false-fired on the `WP-270` concept ref (WP-260-class mis-target). 🟡 **F-127** — kill→resume drill needs a deterministic `debug.killAtStep` seam, not a human racing a 13-min run. **User caveat holds: not a real long build; value was negative-space. NEXT: re-attempt the rehearsal with a calibrated window + kill seam, THEN the 24h exit-gate run.**

> - **Queue:** ✅ **⑦ dogfood-090 (`run-45ce1499`) — the LITERAL-OVERNIGHT consolidation DONE: an autonomous `soak`-paced `chikory run` endured 6h19m / 4 durable parks each 90 min (ABOVE the ~60-min proxy-token TTL) / 6h slept and SEALED SUCCESS, landing the WP-105 F-116 cure (`resolveRunRootContext(runId)` → stable W3C traceId/spanId, no process-local memory). F-116 FIXED + live-proven on a genuine SECOND worker process; token-refresh-across-park SURFACED. Progression ✅ PROGRESSING (rung 4 consolidated).** 🟡 F-117 (root id via private `_spanContext` mutation), 🟡 F-118 (root no longer measures run lifetime) — both track-B under WP-105. ⚠️ **The remaining ⑧ P2-exit axis is now cleanly isolated: endurance-via-idle-soak is proven, but COMPACTION HAS NEVER FIRED live (`peak window 1%` — idle soak parks add zero context; WP-203/204/207 are 🟢 at the mechanism level but never exercised in a headline).** ✅ **⑧ dogfood-091 (`run-7fca16bc`) FALSE-FAILED / dogfood-092 (`run-4481c735`, 🟢 SUCCESS) CURED both launch bugs — F-119 (unsatisfiable AC-2) fixed via the occurrence-count idiom + WP-266 lint, F-120/F-121 (mis-sized/never-armed window) fixed via `CHIKORY_CONTEXT_WINDOW_TOKENS=2000` + the launch preflight (`scripts/dogfood.sh` refuses an unset/executor-scale spec-named env; `CHIKORY_PREFLIGHT_ONLY=1` dry-runs at $0). 092 confirmed the seam reached the workflow (`peak window 115% · compact 4`, first pacing entry 227+1773=2000).** ✅ **⑧ dogfood-093 (`run-60a32aff`, 🟢 SUCCESS · 7 steps, $4.53/$80, 20m52s) — THE FOLD LANDED: WP-251 → 🟢 LIVE-PROVEN.** The F-122 fix (`min_durable_steps: 6` + 7 chunks, SAME ~2000 window) pushed resident summaries past `keepLastN=5` and `planCompaction` FOLDED TWICE (`compactions 2 (pacing 2) · first pacing fold step 5`). The F-123 net-new increment landed too: `pressureFoldGapWarning` (loud-on-silent-fold) + additive `firstPacingFoldStep`. **The ⑧ P2-exit context-rot axis is CLOSED (4-attempt arc 053/091/092/093).** 🟡 F-124 (compaction entry lacks `stepIndex` → inferred first-fold step; track-B under WP-251). ⚠️ **dogfood-094 (`run-b319713b`, 🟢 SUCCESS · 11 steps, $1.95/$40) — user-directed P2 EXIT-GATE REHEARSAL (reactive-signals greenfield build): delivery clean (21/21 tests), but the rehearsal MISSED BOTH target axes.** Window `4000` too big for the small greenfield (proj 1.6–2.6k = 40–65%) → 0 pacing folds (the 3 folds were `trigger:"count"`); the manual kill→resume never fired (`resumes 0`). New: 🔴 **F-125** (window sizing workload-dependent, no calibration — 4th miss); 🟡 **F-126** (stale-spec false-fire on a `WP-270` concept ref); 🟡 **F-127** (kill→resume needs a deterministic seam). ✅ **F-125 + F-127 MECHANISMS HAND-LANDED track-B this sitting (TASK-PROTOCOL §4, uncommitted; 797 TS + 84 py green, tsc/eslint/ruff clean):** (a) **F-125** `pacing.autoCalibrate` — pure `calibrateContextWindow(projectedTokens)` (`src/runner/context-window.ts`) sizes the window from the run's OWN first-step assembled-context tokens (locked once, replay-safe), wired additively into `agent-loop.ts` (absent = unchanged); opt-in YAML field on `PacingPolicy` (TS + py parity); (b) **F-127** the `CHIKORY_KILL_AT_STEP` durable-resume drill seam — after the named step's checkpoint seals, `maybeCrashForResumeDrill` (`activities.ts`) exits 137; the resuming worker (no env) no-ops (pure guard `shouldCrashForResumeDrill`), so the crash fires once; host-injected `debug.killAtStep` (commands.ts), workflow gate rides the frozen input. ✅ **dogfood-095 DONE — BOTH REHEARSAL AXES FIRED (`run-7746e7fa-b16f-47d0-98c0-f99333ecafc6`, 🟢 SUCCESS · 11 steps · $1.77/$40 · 22m29s, uncommitted; `docs/reports/dogfood-095.md`):** the same greenfield reactive-signals build, now on the two landed mechanisms. (a) **AUTO-CALIBRATED PACING FOLD:** `pacing.autoCalibrate` sized the window off step-1 tokens → `peak window 123% · compactions 6 (pacing 6) · first pacing fold step 5` — a REAL `trigger:"pacing"` fold, not count-triggered (vs 094's `pacing 0`). (b) **DETERMINISTIC KILL→RESUME:** `CHIKORY_KILL_AT_STEP=6` exited the worker 137 after the sealed step; `chikory resume` re-entered with ZERO re-execution (journal-verified: an 11-min gap idx36→37 while every other step-gap is ~1 min, no duplicate step entries, clean replay). 20/20 `node --test` green in-workspace (verify §3's AC-1 "fail" is the F-128 wrong-cwd false-alarm). Both axes 094 missed now proven. 🟡 **F-128** — `scripts/dogfood-verify.sh` §3 runs ACs from the repo root not the scaffold workspace → false AC-1 red on `.chikory-examples` runs (track-B verifier hand-fix; not loop-integrity). **NEXT (the only remaining P2 item): the launch-gated 24h+ brownfield EXIT-GATE run — now de-risked (auto-calibrated window + reproducible kill both live-proven).** 🟡 F-126 → track-B (stale-precheck concept-ref mis-target, folds into WP-260/256). **WP-519 + WP-520 (ADR-009 self-heal, RUN level) HAND-LANDED track-B 2026-07-08 — the 24h unattended exit-gate's healer prerequisite now EXISTS (rule-3 HALT → brief+rollback+1 bounded re-judged retry → RESUMABLE FAILED; `chikory resume` re-enters a resumable seal; live-proven `remediation-live.test.ts`, 765 TS + 84 py green).** Track-B follow-ups (never headlines): F-117 → WP-105 span refinement (the private-`_spanContext` mutation needs a provider-level IdGenerator or a documented derived-id contract — an api-only SDK cannot set span ids; still open), F-118 ✅ CLOSED 2026-07-08 (terminal root span now opens at the run row's `started_at` + `run.duration.ms` attr — measures the whole run incl. parks/soaks; `otel.test.ts`), F-116 CLOSED, F-114/F-119 → WP-266/511 (AC-hygiene lint — type-name + grep-`-c`-arithmetic, both landed), F-115 timing-flake convention, F-113 WP-202 eviction knob-exposure, F-110 ✅ CLOSED by WP-520, F-108 ✅ + F-99 ✅ (both found ALREADY LANDED on main — `8a64dbd` / `65ff63b`; the list was stale), WP-515 ✅ LANDED same sitting (F-96 killed-step usage estimation), WP-268 ✅ closed as stale (group-kill at 1× already on HEAD). **Parallel lane (track-B PRs): WP-301 DevAI harness; WP-302 authoring guide.**
...
## 2026-07-04 — displaced from plan.md §6 status block (dogfood-086 review)

Moved verbatim on the dogfood-086 review (WP-271, `run-88235198`, F-107 closed):

- **Last 3 runs (pre-086):** dogfood-083 — WP-270 PER-STEP WORK-UNIT CHUNKING, the rung-3 QUALITY lever (`run-d3879dab`, 🟢 SUCCESS · 3 steps, $4.09/$40): opt-in ordered `work_chunks` + pure `decideWorkChunk` (`src/workflow/work-chunk.ts`) wired into `agent-loop.ts`, additive, LIVE-proven; ✅ F-101 CLOSED by re-run `run-03d161e9` (PROCEED-gated `consumedWorkChunks`). **WP-270 → 🟢** · dogfood-083c (`run-0a285f5b`, 🟢 · 4 steps, $3.02) — accidental glob re-run of the closed WP-270 spec (F-104), harvested `b103645`; WP-214 multi-repo shipped outside the loop (`fadc124`, retires dogfood-084, F-106) · **dogfood-085 — WP-215 ARCHITECTURE RUBRIC + the NON-HOLLOW rung-3 HORIZON (`run-17b5ef57`, 🟢 SUCCESS · 6 steps, $4.36/$50, 22m41s, un-harvested):** the FIRST launch to USE `work_chunks` in its own YAML (5 dependency-ordered sub-goals) — shipped the whole architecture-scan chain (net-new pure `scanDiffForLayeringViolations` → `collectEvidence` `architectureLabels` → `prompt.ts` render → NON-destructive `no_architecture_violations` standing-rubric item → LIVE proof), all 4 ACs green, harvest byte-IDENTICAL, **5/6 non-hollow** (big improvement on 082), **per-step reliability 100%**. **WP-215 architecture side → 🟢.** 🔴 **F-107 → WP-271:** the judge is CHUNK-UNAWARE — at step 2 (chunk 2/5, all ACs+rubric PASS) it ESCALATEd on a "missing" PART 4 that was deferred BY DESIGN → PARKED the run for human approval (no timeout — a HARD blocker for the ⑦ overnight-UNATTENDED rung) and re-handed chunk 2 → hollow step 3 (0-byte, 4.9% cost). 🟡 F-108 (`consumedWorkChunks` not restored on resume, latent).
- **Queue (pre-086):** ✅ **rung-3 machinery COMPLETE — enabler WP-269 (dogfood-081) → hollow horizon proof WP-219 (dogfood-082) → quality lever WP-270 per-step chunking (dogfood-083, `run-d3879dab`, 🟢 SUCCESS, harvest pending).** `work_chunks` + pure `decideWorkChunk` now hand the executor ONE ordered sub-goal per forced step, LIVE-proven; the field EXISTS in HEAD after harvest, so a launch YAML can finally reference it. ⚠️ **dogfood-084 RETIRED** (`.yaml.retired`) — host WP-214 shipped outside the loop (`fadc124`, F-106) so its net-new AC symbol already exists; and the pre-`59c57f6` 3-digit-exact glob mis-fired on its suffixed name → the accidental 083c re-run (F-104; glob + refuse-by-default staleness gate fixed `59c57f6`, F-105). ✅ **⑥ dogfood-085 DONE (`run-17b5ef57`) — the non-hollow rung-3 horizon REACHED on host WP-215 (5/6 non-hollow, 100% per-step reliability).** **NEXT HEADLINE (⛔ STALLED over 083c/085 ⇒ ladder rung BINDING; but ⑦ overnight-UNATTENDED is BLOCKED — §1.2 fallback carve-out FIRES): ⑦′ dogfood-086 — WP-271, the F-107 unblock: chunk-scoped judge adjudication + an unattended-safe ESCALATE policy.** → ⑦ overnight unattended → ⑧ P2 exit-gate 24h brownfield. Track-B follow-ups: F-108, F-99, WP-515 (F-96), WP-512/513/514 (F-91/92/93), F-97 → WP-266/511.
- **Top open friction (pre-086):** 🔴 **F-107 → WP-271 (dogfood-085 — headline blocker):** the judge is CHUNK-UNAWARE. · 🟡 F-108, 🟡 F-109 test-concurrency, ✅ F-100/F-101 CLOSED (WP-270), 🔴 F-99 (WP-206-adjacent), 🟡 F-97/F-98/F-96/F-90/F-85, ✅ F-95 CLOSED, ✅ F-82/F-83/F-84 CLOSED, 🟡 F-58/F-79. (Full text in the git history of plan.md §6.)

## 2026-07-08 — displaced from plan.md §6 status block (WP-519/WP-520 hand-land session)

Moved verbatim when the ADR-009 self-heal run level landed track-B (F-110/F-108/F-99 closed):

- **F-110 (pre-close):** 🟡 **F-110 → track-B (dogfood-086):** the unattended escalate seals `status=FAILED`, conflating a policy-park with a genuine failure (a distinct resumable terminal would reshape the frozen `RunStatus` → needs an ADR); operator distinguishes only via the `unattended … escalation` reason prefix.
- **F-108 (pre-close):** 🟡 **F-108 → WP-206×WP-270 (latent):** `consumedWorkChunks` (`agent-loop.ts:146`) isn't restored on resume → a resumed chunked run replays from chunk 0; now coupled to F-110's resume path.
- **F-99 (pre-close):** 🔴 **F-99 → WP-206-adjacent (track-B):** a bare `chikory resume` missing `OPENAI_COMPAT_BASE_URL` silently loses judge routing, masked as a ~30-min stall — add a fail-fast resume precondition.

## 2026-07-09 — displaced from plan.md §status "Last 3 runs" (dogfood-093 review)

**dogfood-090 — WP-105 DURABLE-ROOT-CONTEXT FIX (the F-116 cure), the ⑦ LITERAL-OVERNIGHT consolidation (`run-45ce1499`, 🟢 SUCCESS · 5 steps, $4.94/$80, 6h19m):** `resolveRunRootContext(runId)` derives a stable W3C traceId/spanId so the span tree stays run-rooted across every park with NO process-local memory — F-116 FIXED and live-proven on a genuine SECOND worker process. 4 parks each 90 min > token-TTL / 6h slept → token-refresh-across-park surfaced (both 089 ⑦-axes closed). WP-105 → 🟢; ⑦ CONSOLIDATED. 🟡 F-117, 🟡 F-118 — track-B.

**dogfood-091 — WP-251 LIVE COMPACTION-FOLD OBSERVATION, the ⑧ attempt (`run-7fca16bc`, 🔴 FAILED · 4 steps, $3.32/$80, false-HALT, NOT harvested):** the 4-part product delivery was CORRECT and full-suite-green in-workspace, but the run false-FAILed on 🔴 F-119 (AC-2 `test "$(grep -rc … src/)" -ge 2` UNSATISFIABLE — multiline → `integer expression expected` → judge budget-waste HALT). Separately 🟡 F-120 — the real run NEVER FOLDED (`peak window 1% · compact 0`): `CHIKORY_CONTEXT_WINDOW_TOKENS=1200000` sized against codex's internal tokens, but the pacing window tracks Chikory's assembled-context tokens (~3k). No rung advanced; F-119 lint added to `dogfood-progression.sh`.

## 2026-07-09 — displaced from DOGFOODING.md header status (dogfood-093 review)

dogfood-092 — WP-251 LIVE COMPACTION-FOLD OBSERVATION re-run (the ⑧ P2-exit context-rot axis), SUCCESS but LIVE FOLD MISSED a 3rd time (`run-4481c735-d3b2-4885-b755-d2ad2c73a551`, `docs/reports/dogfood-092.md`). 🟢 SUCCESS · 4 steps · $2.93/$80 · 9m 46s · uncommitted (harvest byte-IDENTICAL). The 4-part delivery (`describeCompactionPressure` pressure-join reducer + `chikory trace` render + deterministic live Temporal fold test) landed CLEAN + full-suite-green (781 passed), and CURED both 091 launch bugs — window finally sized right (`peak window 115% · compact 4`, first pacing entry 227+1773=2000): F-119 (AC idiom), F-120/F-121 (window sizing + launch-preflight env contract) all CONFIRMED CURED, and no false-HALT fired (WP-268 root-span fix held on the part-4 empty verify diff). BUT the real run STILL folded 0× (`pressure-steps 4 (unfolded 4)`, 0 compaction entries): a 4-step run never accumulates `> keepLastN=5` resident summaries, so `planCompaction` (`compaction.ts:22`) folds nothing on EVERY compact decision — 🟡 F-122, the 3rd distinct miss (053-park / 091-undershoot / 092-too-short). HORIZON length, not window size, was the last knob. 🟡 F-123 — no operator warning when a sized window folds nothing (silent green). Progression ⛔ STALLED (ladder rung 4 × 3 headlines). Fixed by dogfood-093 (`min_durable_steps: 6`).

## 2026-07-20 — displaced DOGFOODING.md header block (dogfood-107 pin)
...
## 2026-07-20 — displaced plan.md §6 status-block Queue prose (dogfood-106 review)

**✅ P3-RUNG-1 CLIMBED — dogfood-105 (`chain-da846d34`, seam `=1`, HALTED 8 steps $8.92, `docs/reports/dogfood-105.md`):** a real 3-node chain force-failed its MIDDLE node → `node_replanned` (evidence-enriched) → **N-B-r1 recovered to SUCCESS**, journal-read (both KPI halves; first chain-recovery tick since 097). Chain did NOT fully seal — bonus node N-C delivered correctly but snagged in an out-of-rubric approve loop (🔴 **F-154**) after a genuine judge true-positive (🟡 **F-153**, AC-3 pinned wrong test file). Six launches to climb, each clearing a wall: **F-148 CLOSED** (`a2abd71` deps in executor clone) + F-149 seam-id (`903be2f`) / F-150 devbox-nest (`6347c86`, inert) / F-151 plan-gate prose literal (`8a0580f`) / F-152 stale git index.lock (`8274c1f`). F-153/F-154 = open WP candidates (hand-fix owed for F-154 approve force-seal). **WP-522 read-only `chikory chain trace` HARVESTED + landed this review (5 files, 941 TS green) → F-144 CLOSED.** (dogfood-104 was the unarmed first attempt, F-146 → WP-531 guard landed 2026-07-16; detail → PLAN-HISTORY.md 2026-07-19.) **NEXT HEADLINE (progression ✅ PROGRESSING — chain-recovery KPI moved 0→1; default = next ladder rung): P3-rung-2 = operator `chikory chain resume` of a sealed-FAILED chain (WP-521c), which first needs the (c) substrate hand-landed AND the F-154 approve force-seal fix (else even a rung-2 test chain loops). The halted `chain-da846d34` is itself a candidate resume substrate.**

## 2026-07-20 — displaced DOGFOODING.md header block (dogfood-106 review)
...
## 2026-07-25 — displaced from plan.md "Top open friction" (bench suite `20260725-130418-chikory` review)

- **Top open friction:** ✅ **F-164 → WP-535 (hermetic judge checks) DONE at the substrate 2026-07-24 (dogfood-112, `run-4bd7ddc0`, landed `15649d1`, `docs/reports/dogfood-112.md`):** judge checks are now hermetic (pure `planCheckSideEffectCleanup` + snapshot/restore around `collectEvidence` — created files deleted, modified restored, executor work untouched, all before `writeCheckpoint`); the `brownfield-003` R2 self-satisfaction path is closed, 9 tests. ⏳ NOT yet proven in the wild (frozen-substrate — the run loaded PRE-fix code; a bench re-run must confirm checkpoint == executor diff on a real task). New friction: 🟡 **F-169** (restore is byte-identical for TEXT only — binary/symlink/mode fidelity + `mkdir -p`/try-catch on the content-restore branch owed before any binary-touching check; the judge caught it pre-land, non-blocking → track-B WP-535 residue) · ✅ **F-170 + F-165 → WP-536 (bench launch family preflight) DONE (hand-landed 2026-07-24, uncommitted):** `benchmarks/harness/src/family-preflight.ts` + `main.ts` wiring echo the resolved `executor·judge·code-model` arm before any task and REFUSE to spend on a directive violation (executor≠gemini, judge non-codex/Claude, or a foreign code-stage routing model — the F-170 `gpt-`/`claude-` at `gemini-cli` case) unless `CHIKORY_BENCH_ALLOW_FAMILY_OVERRIDE=1`; 47 harness tests green. (The gemini-cli foreign-model filter for F-170 already landed in `15649d1`.) F-165 evidence: suite `20260723-222341` ran `claude-code`/`anthropic`, not `gemini-cli` — the default flip (`20a2094` 18:30:29) landed 7 min AFTER the 18:23:41 launch; $2.7075 real Anthropic spend where keyless Gemini ≈$0. · ℹ️ **F-171** (ledger `run` column collides on "112" — bench-suite rows should carry a suite-scoped label, not borrow the next dogfood number → track-B, F-168 sibling). · 🟡 **F-166 (track-B hand-fix, no WP):** benchmark copy-back rewrites relative symlinks to absolute sandbox paths (`.cursorrules`/`CLAUDE.md`/`README.md` → `/Users/…/run-58b48706-…/workspace/AGENTS.md`) → artifact not self-contained + 3 phantom ` M` files in every scope review. · ℹ️ **F-167 (track-B note, F-9 family):** judge passes record real tokens (28,312/901 · 26,173/581) at `costUsd: 0` (`judgeModel.model:"default"`, keyless proxy unpriced) → `judge.max_cost_share` inert and WP-303/304 "cost per success" under-reports. · ✅ **F-159 → WP-533 DONE, LIVE-PROVEN 2026-07-23:** the exact two tasks that sealed FAILED under F-159 now seal SUCCESS at 7/7 (bf-001 2/3→3/3). · ✅ **F-163 → WP-534 parity + `blocked` status LANDED 2026-07-23** (corpus = 2 runnable; per-target node = WP-534). · ℹ️ **F-168 (track-B, no WP):** `scripts/dogfood-progression.sh`'s ⛔ STALLED message hard-codes the RETIRED P2 ladder ("the current **WP-265** ladder rung (**plan.md §6** queue)") though the live phase ladder is **WP-530 / §7** — the `rung` column is already phase-scoped, only the message is not; sibling of F-145 (gate is chain-blind). · 🟡 **F-153/F-154 (dogfood-105) still open track-B.**


## 2026-07-25 — displaced from docs/DOGFOODING.md status block (bench suite `20260725-130418-chikory` review)
...
## 2026-07-26 — displaced from plan.md status block (dogfood-113 review)

Top-open-friction prose for the F-178/F-179 launch-path fixes, verbatim:

- 🔴 **F-178 → FIXED 2026-07-26 (launch-path, dogfood-113 refused at launch, $0 spent):** `packages/sdk-ts/src/taskspec.ts:missingProviderEnv` was capability-blind — it demanded `GEMINI_API_KEY` for `code: { provider: gemini }` even under the **keyless** `gemini-cli` executor, whose adapter authenticates over its own CLI-OAuth session and consumes that routing entry for the MODEL NAME only (`executors/gemini-cli.ts`). The directive-correct arm was therefore unlaunchable on a machine that holds no provider keys by design. Now resolves endpoint capabilities and skips any stage whose capability is `kind: "executor"`; failover choices and non-code stages still require their key. 3 new tests, sdk-ts **982 green**. 🟠 **F-179 → FIXED 2026-07-26 (same launch):** the launcher's `✅ Preflight OK (CHIKORY_PREFLIGHT_ONLY=1) — spec lint, AC dry-run, env contract, and window sizing all pass` inspected only `CHIKORY_*` vars, so a spec `chikory run` was certain to refuse dry-ran GREEN and died only after the SDK rebuild + Temporal probe + proxy probe. `scripts/dogfood.sh` §1c-ter now refuses at $0 (exit 5) with the same provider/env-var names, exempting `openai-compat` (the launcher exports `OPENAI_COMPAT_BASE_URL` itself at step 4) and the `code:` stage under a CLI executor adapter; proven both ways (dogfood-113 passes, a `provider: gemini` review stage refuses).

DOGFOODING.md status block displaced by the dogfood-113 review, verbatim:

**🟢 WP-535 (hermetic judge checks) is LIVE-PROVEN IN THE WILD — bench suite `benchmarks/results/20260725-130418-chikory` (`docs/reports/bench-brownfield-20260725-130418.md`, 2026-07-25).**
First **directive-arm** scored suite: executor `gemini-cli`/`agy`, judge codex proxy (GPT-5), launched by ONE `devbox run bench-run`, zero hand-holding. **7/7 requirements, I-SR/D-SR 100%, 36m 56s.**
`brownfield-001` `run-7f576f26` 3/3 (11 steps, 33m 09s, 3 ROLLBACK + 1 HALT→remediation→recovered — **first self-heal on unseeded third-party code**; 3 live compactions ~20K→~600 tok).
`brownfield-003` `run-c6216ebb` 4/4 (2 steps). F-164 closes: the R4 probe is in **0** checkpoint commits and the grader's R2 matched only the agent's own `default.test.ts`. 🟢 F-165 did not recur (WP-536 preflight held).
New friction: 🔴 **F-172 HAND-FIXED** (`agy --print-timeout` defaults to **5m** and was never passed → every step >5 min silently truncated at **exit 0**; bench steps run `max_seconds: 840`, so **64% of every step horizon was discarded** — see §7) ·
🟡 **F-166 recurrence HAND-FIXED** (root cause `benchmarks/harness/src/adapter.ts:236` `cpSync` missing `verbatimSymlinks`) · 🟠 **F-173** (bf-001 R1 runs `npm install` in a Yarn Berry repo → 3 judge rollbacks over self-inflicted churn → WP-302) ·
🟡 **F-174** (judge opposite verdicts on identical evidence) · ℹ️ **F-175** (grader has no hermeticity wrap → probe survives into the archive) · ℹ️ **F-176** (`toolCalls: 0` on every gemini step) · ℹ️ **F-177** (bf-001 grades an `any` widening clean).
ℹ️ **F-167 ESCALATED:** **$0.0000 metered over 427,888 tokens on BOTH arms** — no priced stage exists on the directive arm, so `judge.max_cost_share`/budget gates are inert and WP-303/304 cost-per-success is uncomputable.
**P3-rung-4 — gate 1 of 3 CLEARED (grading integrity ✅); STILL 2 gates away:** runnable corpus **2/5** (`brownfield-002` `status: blocked`, node≥24 vs devbox 22 → **WP-534 = next headline**, + more pins via WP-302) · no baseline arm ever run (WP-304).
**Launch-path fixes 2026-07-26 (dogfood-113 was refused at launch, $0 spent):** 🔴 **F-178 FIXED** — `missingProviderEnv` demanded `GEMINI_API_KEY` for a `code` stage served by the **keyless** `gemini-cli` executor (that stage supplies only a model name, never routes); now capability-aware. 🟠 **F-179 FIXED** — the launcher's `✅ Preflight OK … env contract` never checked routed-provider keys, so a certain-death launch dry-ran green; `scripts/dogfood.sh` §1c-ter now refuses at $0 (exit 5). See §7.
Progression gate = ✅ **PROGRESSING** after this suite's ledger rows (horizon 3→11) with a standing ⚠️ LADDER PACE warning (rung 3→3→3). ℹ️ F-168 (STALLED msg names the retired P2 ladder) · 🟡 F-153/F-154 · 🟡 F-169 = track-B.
