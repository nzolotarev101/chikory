...
- **S4** — context handoff: WP-237 proved the first local linear handoff;
  WP-239/ADR-007 generalizes it to exact write ownership, deterministic
  serialization, shared-store Git-bundle refs, ordered fan-in, provenance, and
  fail-closed conflicts. WP-203 compacted prose notes remain follow-up work.
