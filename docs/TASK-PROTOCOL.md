...
## 8. Communication and Reporting Guidelines

All status updates, plan edits, task checklists, and handoffs (`HANDOFF.md`) generated by workers must adhere to the following readability conventions:
- **Simplified Explanation First**: Start every status report or handoff with a human-digestible summary explaining what was achieved and what remains, avoiding dense terminology. Explain complex mechanisms clearly.
- **Strict Context Preservation**: Do not omit exact names of files, functions, lines of code, run IDs, commit hashes, or metrics (costs, tokens, duration).
- **Clean Structure**: Format comparative figures, costs, or list of files in markdown tables or bulleted lists. Avoid walls of prose, and use visual status flags (`🟢` Completed, `🟡` In Progress, `🔴` Blocked/Failed) for clarity.
