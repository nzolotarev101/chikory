...
| [`dogfood-021.yaml`](dogfood-021.yaml) | WP-218 pure token-budget gate math — `estimateNextStepTokens` + `tokenBudgetBreached` in `runner/budget.ts` (CG-2/F-9; mirrors the USD gate) | not yet run | — |
