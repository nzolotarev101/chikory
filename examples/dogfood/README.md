...
| [`dogfood-022.yaml`](dogfood-022.yaml) | WP-218 pure token-budget gate math — `estimateNextStepTokens`/`tokenBudgetBreached` in `runner/budget.ts`, the token twin of the USD gate (contract `TaskSpec.budgetTokens` already landed). _(The deferred dogfood-021 slice, now next; watch this run for the retired F-11 probe step.)_ | not yet run | — |
