...
| _(dogfood-022 — WP-218 pure token math — **withdrawn 2026-06-14**)_ | The whole WP-218 token gate (pure math + loop wiring + `budget_event` `cause?`/`remainingTokens?` shape) was hand-landed clearing the architect wall, so a pure-math-only dogfood would clone HEAD and find the work already done. Next dogfood targets a now-unblocked pillar (WP-219 S2 planner impl or WP-203 S2 digest wiring). | withdrawn | — |
