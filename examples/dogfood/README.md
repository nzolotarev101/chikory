...
| [`dogfood-023.yaml`](dogfood-023.yaml) | WP-219 S2 Slice 2 — pure plan-assembly half (`buildPlan(reply, input, opts): Plan`, mirroring the judge's `buildVerdict`; assembles a validated `Plan` from the planner's schema-valid `{ nodes }` reply; JSON parse + id + clock stay in the non-pure `decompose` wrapper) | not yet run | — |
